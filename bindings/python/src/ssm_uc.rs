...
fn set_component<'py>(
    ...
    match c {
        ...
        Some(c) => {
            ...
            d.set_item(format!("{name}_var"), c.smoothed_var.clone().into_pyarray(py))?;
            d.set_item(format!("filtered_{name}"), c.filtered.clone().into_pyarray(py))?;
            d.set_item(
...
/// and smoothed states and variances, residuals, forecasts and forecast
/// variances pinned at 1e-8 against statsmodels for 26 component
/// combinations and NaN-inserted series (fixtures/uc.json); the MLE
...
/// precision; the Harvey-Durbin (1986) UK seat-belt BSM on the vendored
/// Seatbelts data at the statsmodels optimum, with its slope and seasonal
/// variance pile-ups flagged; parameter recovery, forecast-interval
/// coverage and scale invariance measured by seeded Monte Carlo (see the
/// model card).
///
...
/// `se` are observed-information (numerical Hessian); `aic`/`bic` use
