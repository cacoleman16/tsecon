...
**Shipped from this list** (see CHANGELOG 0.3.0 and earlier): Engle-Granger p-values/CVs, Box-Cox λ + `ndiffs`, seasonal ARIMA, DF-GLS, Zivot-Andrews, STL (+ `seasonal_strength` and `nsdiffs`), SETAR + the Hansen sup-F bootstrap linearity test, and EVT tails (GPD/GEV). 0.3.0 also cleared the confirmed-open adversarial-audit backlog (the `lp(cumulative="both")` standard-error defect, `bvar_ssvs` scale non-invariance, the HAC `use_correction` default story, three quantities dropped at the binding, `cv_splits`' inert purge/embargo, and the coverage-page integrity findings).
...
- **VaR backtest battery** (Kupiec/Christoffersen/DQ, S-M, closed-form goldens) — already scoped in Module 03; promoted here because the lab's frontier study (`lab/REPORT.md`) had to hand-roll Kupiec to evaluate tail forecasts.
- **DCS robust local level** (Student-t/Laplace score-driven trend, M) — the lab's strongest graduation candidate (−22%/−31% RMSE under 5/10% contamination, zero clean-data tax): Gaussian-limit golden vs statsmodels UnobservedComponents + MC-recovery grading, extending the shipped `gas_volatility` family.
- **Split-panel-jackknife panel LP** (S-M, R golden from the Mei-Sheng-Shi repo) — top-ranked in the research-contribution scan (`docs/roadmap/19-research-contributions.md`) and the principled completion of the `panel_lp` jackknife caveat 0.3.0 documented.
