...
**Phases 0–1 complete; Phases 2–4 substantially landed.** 28 Rust crates implemented (~37,000 source lines), **368 Rust tests + 85 Python tests, all green and golden-fixture-gated** against statsmodels, SciPy, NumPy, `arch`, `linearmodels`, `scikit-learn`, `ArviZ`, and `scipy.signal`. Everything installs and runs from Python today. The repository builds from a clean clone (CI-verified), with `cargo fmt`/`clippy`/`test` and a build-and-test-the-wheel matrix (Linux/macOS/Windows) on every push, plus a tag-triggered PyPI release pipeline with trusted publishing.

**Built and callable from Python now** (63 functions): the diagnostic battery (ACF/PACF, Ljung-Box, Jarque-Bera, ARCH-LM) and the full unit-root workflow (ADF, KPSS, `check_stationarity`); robust standard errors (HAC/Newey-West with a uniform `se_type=`); the bootstrap family; the exact-diffuse Kalman filter/smoother; **ARIMA** (exact MLE + forecasting), **GARCH/GJR/EGARCH** (QMLE + robust SEs), **VAR** (IRF/FEVD/Granger/forecast, with cumulative IRF views); trend-cycle filters; forecast evaluation (Diebold-Mariano, Clark-West, Giacomini-White, Theta, accuracy measures) and the **rolling/expanding backtest engine** (`backtest`); a **Minnesota-NIW Bayesian VAR** with posterior IRF draws and convergence diagnostics; the dedicated **local projections** module (`lp`/`lp_iv` — lag-augmented default, LP-IV with first-stage F, cumulative); **penalized regression** (ridge/lasso/elastic-net, plus `adaptive_lasso` and the AIC/BIC-selected `lasso_path`) with **leakage-safe time-series CV** (`cv_splits` — expanding/rolling/purged-K-fold); **sign-restricted Bayesian SVARs** (`sign_restricted_svar` — identified-set bands + acceptance diagnostics), the headline structural-identification differentiator; **IV-GMM** (`iv_gmm` — 2SLS/two-step/iterated, robust or HAC weighting, Hansen J-test); the first **panel** slice (`panel_fe`/`panel_lp` — clustered/Driscoll-Kraay SEs, cumulative + jackknife); **spectral analysis** (`periodogram`/`welch`/`coherence`); **cointegration** (`johansen`/`vecm`); **Markov-switching AR** (`markov_switching_ar`, the Hamilton model); **MIDAS** mixed-frequency regressions (`midas_weights`/`umidas`); **multivariate GARCH** (`ccc_garch`/`dcc_garch` — Bollerslev CCC and Engle DCC); **realized volatility** (`realized_measures` — RV/bipower/jump per BNS 2004, and `har_rv` — Corsi HAR with HAC SEs); **Diebold-Yilmaz connectedness** (`connectedness` — the spillover table from a VAR's generalized FEVD); the **PCA factor model** (`factor_model` — factors/loadings with Bai-Ng and Ahn-Horenstein factor selection); and the **Nelson-Siegel/Svensson** yield curve (`nelson_siegel`/`svensson`, with NLS-optimal decay).

**Built in Rust, awaiting Python bindings** (validated, committed): weighted-NLS MIDAS (Module 08); state-dependent local projections (Module 07); the Haar rotation kernel and non-Bayesian identification machinery (Module 06); the mean-group panel VAR (extension E1); the two-step **dynamic Nelson-Siegel** and the **FAVAR** two-step IRF mapping (Module 04, factor core already bound); and the general nonlinear GMM driver (`gmm_nonlinear`, linear IV-GMM already bound).

**Documentation shipped**: a 13-chapter [teaching guide](docs/guide/README.md) (beginner to research-grade, including nonlinear dynamics), a worked figure [gallery](docs/examples/README.md) (base, advanced, and structural sections), this roadmap's 15 module specs, and an interactive demo.

**Next up**: wire the remaining built modules above into Python; then the remaining Phase-3/4 depth — mixed-frequency DFM nowcasting (the NY-Fed architecture, Module 08), predictive regressions / IVX (E3, pending a validated IVX reference), panel time series PMG/CCE (E1), and the DSGE-lite decision (E5). IV-GMM, realized-volatility measures, FAVAR, Diebold-Yilmaz connectedness, and the forecast backtester (Modules 03–04, 09, 10) are now built and bound. The library name is still the working codename `tsecon` and must be resolved before the first PyPI upload (Module 11/14).
...
- **Naming**: shortlist to vet for PyPI/trademark/SEO — *impulse*, *kairos*, *tempora*, *metrika*, *macrodyn*, *chronometrics* (avoid *chronos*: Amazon's model). Decide before first public release; the name goes in the JOSS paper title. Working codename until then.
