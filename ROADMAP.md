...
- **ACM** (Adrian-Crump-Moench) regression-based affine term premium (M) — extends the term-structure crate with a strong closed-form/OLS golden.
- **Static copulas** (Gaussian/Student-t/Archimedean, M, vs `scipy.stats`/`statsmodels` where expressible) — the E12 slice; EVT (E11) shipped in 0.3.0.
- **MSTL** (M, vs `statsmodels.tsa.seasonal.MSTL`) — unblocked now that STL is pinned elementwise.
- **GLP-exact residual-scale convention** (S) — `bvar_fit`/`bvar_hierarchical` gain a `scale_ar="ar1"` option matching Giannone-Lenza-Primiceri's own σ²ⱼ convention: the GLP design replication showed the AR(4)-vs-AR(1) convention is the entire gap to their published Figure-1 tightness modes (switching it reproduces 0.449/0.172 on their data), so this one small option turns a design replication into a point replication.
