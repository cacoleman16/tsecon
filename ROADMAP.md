...
**Built and callable from Python now** (38 functions): the diagnostic battery (ACF/PACF, Ljung-Box, Jarque-Bera, ARCH-LM) and the full unit-root workflow (ADF, KPSS, `check_stationarity`); robust standard errors (HAC/Newey-West with a uniform `se_type=`); the bootstrap family; the exact-diffuse Kalman filter/smoother; **ARIMA** (exact MLE + forecasting), **GARCH/GJR/EGARCH** (QMLE + robust SEs), **VAR** (IRF/FEVD/Granger/forecast, with cumulative IRF views); trend-cycle filters; forecast evaluation (Diebold-Mariano, Theta, accuracy measures); a **Minnesota-NIW Bayesian VAR** with posterior IRF draws and convergence diagnostics; the dedicated **local projections** module (`lp`/`lp_iv` — lag-augmented default, LP-IV with first-stage F, cumulative); **penalized regression** (ridge/lasso/elastic-net); and **sign-restricted Bayesian SVARs** (`sign_restricted_svar` — identified-set bands + acceptance diagnostics), the headline structural-identification differentiator.

**Built in Rust, awaiting Python bindings** (validated, committed): the **forecast backtesting engine** + Clark-West + Giacomini-White (Module 09); the rest of Module 10's ML — leakage-safe time-series CV, adaptive lasso, regularization paths; state-dependent local projections (Module 07); the Haar rotation kernel and non-Bayesian identification machinery (Module 06); and the first **panel** slice — fixed effects with clustered/Driscoll-Kraay SEs, panel LP, mean-group VAR (extension E1).
