...
**Built in Rust, awaiting Python bindings** (validated, committed): the `smooth_fixed` fixed-parameter DFM state-space entry point (Module 08, used internally by `dfm_nowcast`). The built-but-unbound backlog is otherwise drained — the Module 06 identification internals are now all bound.
...
**Build-later — valuable but weaker validation or larger surface:** Hansen (1997/2000) threshold confidence sets for SETAR, regime-dependent generalized impulse responses (Koop-Pesaran-Potter) for the shipped threshold VAR, JSZ canonical affine term structure, and distributed-lag climate-impact regressions. (LP-DiD, the copula tail-dependence coefficients, the joint-across-horizon LP bands, the Ng-Perron M-tests, the restricted VECM deterministic cases — `"ci"`/`"li"`/`"lo"`, the four combinations, and centered seasonal dummies, all statsmodels-golden-pinned per case with the widened `beta` returned as statsmodels' `det_coef_coint` split, finishing field-report item 12 — threshold cointegration (Hansen-Seo) with the threshold VAR (`threshold_vecm`/`hansen_seo_test`/`threshold_var`/`threshold_var_test` — transcription + Monte-Carlo grade, tsDyn unreachable in the build container), and the STAR family (`star`/`star_eval`/`star_test` — transcription-pinned Teräsvirta modeling cycle, MC-graded estimator), formerly on this list, have all shipped.)
...
**Also on deck**: the remaining §9 adoption polish (head-to-head explainer chapters, a public speed dashboard, more published replications).
