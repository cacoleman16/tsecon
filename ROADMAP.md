...
had a runnable golden has shipped. Of the streams interrupted mid-wave by the
usage limit, the user-reported repairs, the conformal interval wrappers, and the
DCC build-out were resumed, verified, and shipped in 0.5.0; two remain preserved
on `claude/wip-0.5.0-*` branches for 0.6.0, unverified and ungated — the
interval-coverage expansion (its agent modified the coverage page's guarded
