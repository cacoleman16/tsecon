...
**Phases 0–1 complete; Phases 2–4 substantially landed.** 27 Rust crates implemented (~36,000 source lines; a 28th, GMM/IV-GMM, is in flight), **358 Rust tests + 72 Python tests, all green and golden-fixture-gated** against statsmodels, SciPy, NumPy, `arch`, `linearmodels`, `scikit-learn`, `ArviZ`, and `scipy.signal`. Everything installs and runs from Python today. The repository builds from a clean clone (CI-verified), with `cargo fmt`/`clippy`/`test` and a build-and-test-the-wheel matrix (Linux/macOS/Windows) on every push, plus a tag-triggered PyPI release pipeline with trusted publishing.

**Built and callable from Python now** (58 functions): the diagnostic battery (ACF/PACF, Ljung-Box, Jarque-Bera, ARCH-LM) and the full unit-root workflow (ADF, KPSS, `check_stationarity`); robust standard errors (HAC/Newey-West with a uniform `se_type=`); the bootstrap family; the exact-diffuse Kalman filter/smoother; **ARIMA** (exact MLE + forecasting), **GARCH/GJR/EGARCH** (QMLE + robust SEs), **VAR** (IRF/FEVD/Granger/forecast, with cumulative IRF views); trend-cycle filters; forecast evaluation (Diebold-Mariano, Clark-West, Giacomini-White, Theta, accuracy measures); a **Minnesota-NIW Bayesian VAR** with posterior IRF draws and convergence diagnostics; the dedicated **local projections** module (`lp`/`lp_iv` — lag-augmented default, LP-IV with first-stage F, cumulative); **penalized regression** (ridge/lasso/elastic-net); **sign-restricted Bayesian SVARs** (`sign_restricted_svar` — identified-set bands + acceptance diagnostics), the headline structural-identification differentiator; the first **panel** slice (`panel_fe`/`panel_lp` — clustered/Driscoll-Kraay SEs, cumulative + jackknife); **spectral analysis** (`periodogram`/`welch`/`coherence`); **cointegration** (`johansen`/`vecm`); **Markov-switching AR** (`markov_switching_ar`, the Hamilton model); **MIDAS** mixed-frequency regressions (`midas_weights`/`umidas`); **multivariate GARCH** (`ccc_garch`/`dcc_garch` — Bollerslev CCC and Engle DCC); **realized volatility** (`realized_measures` — RV/bipower/jump per BNS 2004, and `har_rv` — Corsi HAR with HAC SEs); **Diebold-Yilmaz connectedness** (`connectedness` — the spillover table from a VAR's generalized FEVD); the **PCA factor model** (`factor_model` — factors/loadings with Bai-Ng and Ahn-Horenstein factor selection); and the **Nelson-Siegel/Svensson** yield curve (`nelson_siegel`/`svensson`, with NLS-optimal decay).

**Built in Rust, awaiting Python bindings** (validated, committed): weighted-NLS MIDAS (Module 08); the **forecast backtesting engine** (Module 09); the rest of Module 10's ML (leakage-safe time-series CV, adaptive lasso, regularization paths); state-dependent local projections (Module 07); the Haar rotation kernel and non-Bayesian identification machinery (Module 06); the mean-group panel VAR (extension E1); the two-step **dynamic Nelson-Siegel** and the **FAVAR** two-step IRF mapping (Module 04, factor core already bound).
...
**Next up**: finish and wire **GMM/IV-GMM** (Hansen J-test, in flight); wire the modules above into Python; then the remaining Phase-3/4 depth — mixed-frequency DFM nowcasting (the NY-Fed architecture, Module 08), predictive regressions / IVX (E3, pending a validated IVX reference), panel time series PMG/CCE (E1), and the DSGE-lite decision (E5). Realized-volatility measures, FAVAR, and Diebold-Yilmaz connectedness (Modules 03–04) are now built and bound. The library name is still the working codename `tsecon` and must be resolved before the first PyPI upload (Module 11/14).
