...
**Build-later — valuable but weaker validation or larger surface:** STAR/LSTAR/ESTAR, threshold cointegration (Hansen-Seo) and threshold VAR, Hansen (1997/2000) threshold confidence sets for SETAR, JSZ canonical affine term structure, and distributed-lag climate-impact regressions. (LP-DiD, the copula tail-dependence coefficients, the joint-across-horizon LP bands, the Ng-Perron M-tests, and the restricted VECM deterministic cases — `"ci"`/`"li"`/`"lo"`, the four combinations, and centered seasonal dummies, all statsmodels-golden-pinned per case with the widened `beta` returned as statsmodels' `det_coef_coint` split, finishing field-report item 12 — formerly on this list, have all shipped.)
**Build-later — valuable but weaker validation or larger surface:** STAR/LSTAR/ESTAR, Hansen (1997/2000) threshold confidence sets for SETAR, regime-dependent generalized impulse responses (Koop-Pesaran-Potter) for the shipped threshold VAR, JSZ canonical affine term structure, distributed-lag climate-impact regressions, and the restricted VECM deterministic cases (`"ci"`/`"li"`/`"lo"` and seasonal dummies — `vecm` ships `"n"` and `"co"`; the restricted cases put deterministic terms *inside* the cointegration relation, widening `beta` and the normalization, and statsmodels is a runnable golden for each when built). (LP-DiD, the copula tail-dependence coefficients, the joint-across-horizon LP bands, the Ng-Perron M-tests, and threshold cointegration (Hansen-Seo) with the threshold VAR (`threshold_vecm`/`hansen_seo_test`/`threshold_var`/`threshold_var_test` — transcription + Monte-Carlo grade, tsDyn unreachable in the build container), formerly on this list, have all shipped.)
