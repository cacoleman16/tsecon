...
- **Engle-Granger cointegration p-values/CVs** (S) — `engle_granger.rs` ships statistic-only with a TODO; the MacKinnon surfaces are already transcribed, and `statsmodels.tsa.stattools.coint` gives a full stat/p/CV golden. Cheapest high-leverage item.
- **DF-GLS** (Elliott-Rothenberg-Stock, M) and **Zivot-Andrews** (one endogenous break, M) — both pin exactly against `arch.unitroot` (Zivot-Andrews doubly, `arch` and `statsmodels` agreeing). DF-GLS also builds the GLS-detrending engine Ng-Perron reuses.
- **Box-Cox λ selection** (S, vs `scipy.stats.boxcox`/`boxcox_normmax`) and **`ndiffs`** differencing advisor (S, a thin composition over the shipped ADF/KPSS/PP) — both also sharpen `check_series`.
- **SARIMA/SARIMAX** (M, vs `statsmodels` SARIMAX) and **STL** decomposition (L, vs `statsmodels.tsa.seasonal.STL`) — STL then unblocks seasonal-strength detection and `nsdiffs`.
- **SETAR/TAR** threshold autoregression (M) — the workhorse nonlinear model, extending the Markov-switching/state-dependent-LP nonlinearity already shipped.
...
- **EVT** (POT/GPD + GEV block-maxima, M, vs `scipy.stats.genpareto`/`genextreme`) and **static copulas** (Gaussian/Student-t/Archimedean, M) — the first defensible E11-E12 slices.

**Build-later — valuable but weaker validation or larger surface:** Ng-Perron M-tests (table+MC golden), MSTL, `nsdiffs`/seasonal-strength (need STL first), auto.arima stepwise order selection (large; the famous pmdarima-vs-R-parity trap — will be graded MC-recovery, not R-parity), STAR/LSTAR/ESTAR, threshold cointegration (Hansen-Seo) and threshold VAR, JSZ canonical affine term structure, tail-dependence coefficients, and distributed-lag climate-impact regressions.
