...
**Next up**: the remaining Phase-3/4 depth — predictive regressions / IVX (E3, pending a validated IVX reference), pooled mean-group (PMG) ARDL to complete the panel-time-series trio (E1, MG + CCE-MG landed), one-step MLE DFM and the news/update decomposition on top of the two-step nowcaster, and the DSGE-lite decision (E5). GAS score-driven volatility, DFM nowcasting, panel MG/CCE-MG, IV-GMM, realized-volatility measures, FAVAR, and Diebold-Yilmaz connectedness are all now built and bound. The library name is still the working codename `tsecon` and must be resolved before the first PyPI upload (Module 11/14).
