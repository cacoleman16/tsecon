...
**Next up**: wire the modules above into Python; then the remaining Phase-3/4 depth — mixed-frequency DFM nowcasting (the NY-Fed architecture, Module 08), realized-volatility measures and jump tests (Module 03), FAVAR and Diebold-Yilmaz connectedness (Module 04), and the DSGE-lite decision (E5). The library name is still the working codename `tsecon` and must be resolved before the first PyPI upload (Module 11/14).
