...
One scoping note before the tour. Chapter 4 already introduced the univariate members of this family — Markov-switching AR, where a latent regime switches the mean and dynamics of one series, and SETAR/STAR, where an observable threshold does — and all three of them ship today, as `tsecon.markov_switching_ar`, `tsecon.setar` and `tsecon.star`. Everything there carries over: the Davies problem in linearity testing, the multimodal likelihoods, the regime probabilities as deliverable — and one warning from that chapter's roadmap becomes a theme here: for nonlinear models, even *forecasting* requires simulation, because iterating the fitted conditional mean forward plug-in style is biased beyond one step. The impulse-response version of that fact is this chapter's centerpiece. What chapter 4 could not cover is what this chapter owns: **systems** — nonlinear VARs and their multi-variable interactions — and above all **impulse responses**, the object nonlinearity damages most.

One inference note, because it is where the univariate chapter stopped short and the question comes back for every threshold model in this one. The SETAR threshold estimate is superconsistent — it converges at rate $n$, not $\sqrt{n}$ — but its sampling distribution is nonstandard, so the coefficient standard errors `setar` reports say nothing about how well $\gamma$ itself is pinned down. Hansen (1997, 2000) answered with a likelihood-ratio inversion: compute $\mathrm{LR}_n(\gamma) = n\,(S_n(\gamma) - S_n(\hat\gamma)) / S_n(\hat\gamma)$ over the candidate grid and keep every $\gamma$ at which it stays below a closed-form critical value ($7.35$ at 95%, from $P(\xi \le x) = (1 - e^{-x/2})^2$). The result is a confidence *set*, not an interval: it always contains $\hat\gamma$, is asymmetric, and can split into several pieces when the sum-of-squares profile has more than one near-minimal valley — which is exactly the information you want, because a set that covers half the grid is the honest way of saying the threshold is weakly identified. `tsecon.setar_threshold_ci` builds it on exactly the `setar` fit (bit-identical grid and profile), returns the pieces as `intervals` with the convex hull alongside, applies Hansen's $\eta^2$ scaling under `het_robust=True`, and unions the conventional slope intervals over an 80% threshold region when you ask for `slope_level` — his conservative construction for the regime coefficients. Its coverage is measured, not assumed: on a threshold regression after Hansen's own Monte Carlo design and on a SETAR(2), the 95% set covers the true threshold 0.93–0.98 of the time from $n = 100$ to $500$ (500 replications per cell, in the [model card](../reference/model-cards/cointegration-regime.md#setar_threshold_ci-hansen-19972000-threshold-confidence-set)), at or above nominal except for a small effect in the shortest sample — the asymptotically-conservative behaviour the theory predicts for a fixed threshold effect. The same caveat the Davies problem taught applies in reverse here: the set presumes a threshold exists, so run `setar_test` first; on linear data it simply returns most of the grid.
...
Step 3's trick — common random numbers across the paired runs — plus antithetic variates in step 2 are classic variance-reduction devices, and they matter because the raw computational bill is (histories) × (shock-path replications) × (horizons) × (cost per simulation step), *before* you add a bootstrap loop around the whole thing for confidence bands. In an interpreted loop this takes hours, which is why published GIRF bands are often thinner on replications than anyone would like and why the method has a reputation for pain. The structure, though, is embarrassingly parallel — thousands of independent simulations that never communicate — which is precisely the shape of problem a compiled core with counter-based parallel RNG dispatches in seconds. This is the same speed story the bootstrap chapters have been telling, and it is why tsecon builds the KPP engine as shared infrastructure, once, for every nonlinear model in the library: given any model that can simulate forward from a state, the engine computes $GIRF(h, \delta, \text{history set})$ with variance reduction and reproducible parallelism. `tsecon.threshold_var_girf` is that engine pointed at the fitted threshold VAR, and `tsecon.var_girf` is the same engine pointed at a linear VAR — where the answer is known in closed form, which is how the engine is validated (it reproduces `var_irf(orth=True)` and the Pesaran-Shin generalized IRF to 1e-12 with a single draw).

Here is the toy's system version through the library: a bivariate TVAR(1) whose persistence switches at a threshold on the first variable, with the *same* innovation covariance in both regimes so the regime comparison is about dynamics alone. One call returns the average GIRF, its split by the regime each history was in when the shock hit, the spread across histories, and the Monte Carlo error of all of it:
...
y = np.zeros((600, 2))
for t in range(1, 600):                          # persistent below 0, transient above
...
girf = tsecon.threshold_var_girf(y, size=1.0, **kw)       # fits the TVAR, then simulates
...
print(f"threshold {girf['threshold']:+.3f}; histories {girf['n_histories']} "
      f"({girf['n_low_histories']} low, {girf['n_high_histories']} high); "
      f"raw shock {girf['shock_size_used'][0]:.3f} / {girf['shock_size_used'][1]:.3f}")
print(" h   low-regime  high-regime  linear VAR   (response of y0 to its own 1-sd shock)")
for h in range(9):
    print(f" {h}     {low[h, 0]:+.3f}      {high[h, 0]:+.3f}      {lin[h, 0]:+.3f}")
...
print("h=2  GIRF(+1) %+.3f  -GIRF(-1) %+.3f  GIRF(+3)/3 %+.3f  MC se %.4f"
      % (g[2, 0], -neg[2, 0], big[2, 0] / 3, girf["mc_se"][2][0]))
```

```
threshold -0.114; histories 399 (288 low, 111 high); raw shock 0.597 / 0.559
 h   low-regime  high-regime  linear VAR   (response of y0 to its own 1-sd shock)
 0     +0.597      +0.559      +0.605
 1     +0.420      +0.181      +0.434
 2     +0.320      +0.112      +0.320
 3     +0.251      +0.080      +0.241
 ...
 7     +0.108      +0.031      +0.087
 8     +0.088      +0.025      +0.068
h=2  GIRF(+1) +0.262  -GIRF(-1) +0.336  GIRF(+3)/3 +0.179  MC se 0.0003
```

Read it against the toy. **History**: the same one-standard-deviation shock has decayed to 0.42 after a period when it lands in the persistent regime and to 0.18 when it lands in the transient one — and the linear VAR, which averages the two laws of motion in proportion to how often the sample visited them (72% low), tracks the low-regime answer and simply *misses* the high-regime economy. **Sign** (last line): mirrored, the negative shock lingers more than the positive one (0.336 against 0.262 at $h = 2$) — it pushes histories deeper into the persistent regime, the positive one pushes them across the threshold into the transient one. **Size**: per unit of shock the $3\sigma$ impulse dies faster (0.179 against 0.262) because it carries more histories across the threshold at once. And the Monte Carlo error on all of this is 0.0003 with 500 antithetic draws per history — the roadmap's showcase configuration (600 observations, three variables, 200 histories, 500 draws, horizon 20 — 4.2 million simulated periods) runs in a quarter of a second in the compiled engine and 0.6 s end to end from Python including the fit, on four cores. Two honest notes the model card spells out: the *raw* size of "one standard deviation" is set by the covariance of the regime the history is in at the shock date (`shock_size_used` reports both, 0.597 and 0.559 here, because the two regime covariances are *estimated* separately even though the DGP's are equal); and antithetic pairs, the textbook variance-reduction device, buy essentially nothing on a threshold model (measured variance ratio 1.00) — the paired difference is dominated by regime-crossing events, which are not odd functions of the innovation. More draws are what shrink `mc_se`.

If the toy above ran in a quarter of a second, where do the hours go? Scale each factor honestly: an estimated TVAR replaces the one-line `step` with a matrix recursion over several variables; one conditioning history becomes hundreds (every recession date in the sample); and — the multiplier that hurts — the GIRF you computed (and the one `threshold_var_girf` returns) is a *point estimate*, conditional on the estimated parameters. Its uncertainty has two layers: **simulation noise**, which more draws kill (with $R = 20{,}000$ paired paths it is already negligible above), and **estimation uncertainty**, which no number of draws touches — the thresholds, coefficients, and covariances are themselves estimated. Honest GIRF bands therefore wrap a bootstrap around the entire engine: re-estimate the model on each bootstrap sample, rerun the full history-by-history simulation on each re-estimate, and read bands off the distribution. That is the algorithm above times a few hundred model refits, and it is the step published papers most often skimp on — thin replications, or bands that account for simulation noise only. It is also, being independent replications all the way down, exactly the workload a parallel compiled core is for.
...
The loop is spelled out because the design matrix *is* the lesson. In production, `tsecon.lp_state(y, eps, I, horizons=12, n_lag_controls=1)` is the same estimator in one call — it lags the indicator for you, interacts impulse and controls with it and its complement, and returns `irf_state1`/`irf_state0` with their standard errors; on this DGP it lands on the same impact responses, 1.96 and 0.95. Chapter 9 covers its standard-error default (lag augmentation, with the `cumulative="both"` carve-out) and which simultaneous bands it will and will not build.
...
|---|---|---|
| First pass at "does the response differ by regime?" | State-dependent LP with a lagged indicator (`lp_state`) | Interactions are almost free; no transition model to misspecify |
...
| Named observable trigger for system dynamics (spread, inflation) | `threshold_var`, linearity first via `threshold_var_test`; GIRFs by `threshold_var_girf` | Interpretable threshold; complete law of motion for counterfactuals |
| Latent phases you want the system to discover | MS-VAR (Krolzig taxonomy) — one series at a time, `markov_switching_ar` | Regime probabilities are the deliverable; budget for estimation fragility |
| The regime lives in the cointegrating relation, not the levels | `threshold_vecm`, tested with `hansen_seo_test` | Arbitrage bands and adjustment costs make error correction kick in only when the gap is wide |
...
| Any IRF from any nonlinear model | KPP generalized IRFs — simulate sizes, signs, history sets (`threshold_var_girf`; `var_girf` as the linear comparator) | Size/sign/history invariance is gone; the GIRF arguments are the analysis |
...
**Available now in Python** — including this chapter's headline regime models:

- `tsecon.threshold_var`, `tsecon.threshold_var_test`, `tsecon.threshold_var_girf` — the two-regime TVAR of the first section: the concentrated log-determinant scan over $(\tau, d)$, the robust score-form sup-Wald linearity test p-valued by Hansen's fixed-regressor wild bootstrap (seeded and parallel, so the p-value is bit-identical at any thread count), and the regime-dependent Koop-Pesaran-Potter GIRFs of the load-bearing section — history-conditional, split by regime, at any shock size and sign, with the across-history distribution and the Monte Carlo error returned alongside the average. Point estimates conditional on the fit: bootstrap-over-refits bands are the roadmap item below
- `tsecon.var_girf` — the same simulation engine pointed at a linear VAR, which is how it is validated (it reproduces `var_irf(orth=True)` and the Pesaran-Shin generalized IRF to 1e-12) and the linear comparator to put next to any nonlinear GIRF
- `tsecon.setar`, `tsecon.setar_test` — the univariate ancestor: two-regime SETAR by concentrated least squares, with the Hansen (1996) sup-F linearity test on the same bootstrap. Replicated against Hansen's (1999) sunspot fit — delay 2, threshold rounding to the published 7.4, linearity rejected at 5% ([replication](../examples/replication-setar-sunspots.md)); `tsecon.setar_threshold_ci` adds the Hansen (1997/2000) likelihood-ratio confidence set for the threshold on exactly that fit — closed-form critical values, a possibly disjoint set reported as intervals, the η² heteroskedasticity scaling and the conservative slope-interval union — with its coverage measured by Monte Carlo rather than assumed
- `tsecon.star`, `tsecon.star_eval`, `tsecon.star_test` — the smooth-transition family the STVAR section is the system version of: LSTAR and ESTAR by concentrated NLS over a standardized-γ grid, the fixed-$(\gamma, c)$ scorer, and the full Teräsvirta modeling cycle (the LM3 linearity test plus the H03/H02/H01 sequence that chooses between LSTAR and ESTAR). The weak identification of γ this chapter warned about is *reported* rather than hidden: `gamma_at_boundary`, `se_valid` and `converged` come back with every fit
- `tsecon.markov_switching_ar` — Hamilton (1989) MS-AR by EM, the univariate member of the MS-VAR family: the transition matrix, per-regime means and variances, `expected_durations`, and the `filtered_prob`/`smoothed_prob` regime paths that are the deliverable. It reproduces Hamilton's published GNP estimates and his recession chronology ([replication](../examples/replication-hamilton-markov.md)) — and note the timing rule that comes with it, that `smoothed_prob` conditions on the whole sample and is therefore not a real-time recession call; `filtered_prob` is
- `tsecon.threshold_vecm`, `tsecon.hansen_seo_test` — nonlinearity in the *cointegrating* relation: the Hansen-Seo (2002) two-regime VECM in which the error-correction term itself drives the regime split, with the sup-LM test of linear against threshold adjustment on the same fixed-regressor bootstrap. Chapter 7's territory, this chapter's kind of model
- `tsecon.lp_state` — the state-dependent LP hand-rolled above, as one call: impulse and controls interacted with the lagged regime indicator, one IRF and standard-error path per regime (chapter 9)
...
- `tsecon.philox_uniforms`, `tsecon.bootstrap_indices`, `tsecon.optimal_block_length` — counter-based parallel RNG and block-resampling primitives: the exact ingredients the KPP simulation engine is built from. The first GIRF demo above deliberately used nothing but NumPy — the *concept* is thirty lines; what `threshold_var_girf` adds is estimated-model integration, common random numbers, per-(history, draw) Philox substreams, and the speed to make all of that routine
...
**One grading note**, because this is where the library's validation ladder is shortest and the difference matters when you cite a number. `markov_switching_ar` is pinned against statsmodels' `MarkovAutoregression` and then against Hamilton's published table; `setar` adds the Hansen (1999) sunspot replication. The rest of the threshold and smooth-transition block — `star`, `threshold_var`, `threshold_vecm` and their tests — ships at a **transcription plus Monte-Carlo grade**, not a third-party-golden one: every statistic is pinned at 1e-10 against an independent NumPy transcription of the published algorithm, and estimator recovery and bootstrap null size are *measured* by seeded Monte Carlo (for the TVAR, threshold median absolute error 0.008 and coefficient biases within 0.009 at $T = 400$; for its linearity test, rejection 0.100 at a nominal 5% at $T = 150$ falling to 0.085 at $T = 400$ — mildly liberal in small samples, which the model card reports rather than smooths over). What is missing is a second implementation to disagree with: R's `tsDyn` needs CRAN, which the build container cannot reach, and statsmodels has neither a STAR nor a threshold VAR. The reference runs are named follow-ups, and until they happen the model cards grade these functions where they actually stand.

One disambiguation, since the name collision is unfortunate: `tsecon.hamilton_filter` is Hamilton's (2018) regression-based *detrending* filter (the modern HP-filter alternative, chapter 7's territory) — not the Hamilton (1989) *Markov-switching* filter this chapter discussed. That one ships too, inside `tsecon.markov_switching_ar`, which returns its filtered probabilities directly rather than exposing the filter as its own entry point.

**Roadmap** — what the shipped regime models above still do not cover; the specs are written and tiered:

- **GIRF bands** ([docs/roadmap/04-multivariate.md](../roadmap/04-multivariate.md), Tier 3): the shared Koop-Pesaran-Potter engine ships (`threshold_var_girf`, `var_girf`, and the Rust `tsecon_var::girf` trait any simulable model can implement), but its responses are point estimates conditional on the fitted parameters. The bootstrap-over-refits bands the section above describes — re-estimate the TVAR on each bootstrap sample, rerun the whole history-by-history simulation, read bands off the distribution — are the remaining piece
- **The nonlinear VARs of the middle sections** (same module, Tiers 3–4): STVAR with the calibrated-γ convention, validated against the Auerbach-Gorodnichenko replication files; MS-VAR across the Krolzig taxonomy; interacted VARs in Tier 4. For the TVAR that already ships, the Balke (2000) reference run is a named follow-up, as are Hansen (1997/2000) threshold confidence sets for SETAR
- **Nonlinear local projections** ([docs/roadmap/07-local-projections.md](../roadmap/07-local-projections.md)): `lp_state` ships the regime-indicator design, and the Gonçalves-et-al. estimand warning is taught rather than automated. Still open: the IV version for state-dependent fiscal multipliers, the Ramey-Zubairy *state-dependent* validation (the shipped replication pins their linear multiplier), the smooth-transition LP, and sign/size-dependent LP with joint asymmetry tests. Time-varying LP is what remains of the frontier tier
- **Linearity screens** ([docs/roadmap/02-univariate.md](../roadmap/02-univariate.md)): the Tsay arranged-autoregression test this chapter recommends as a cheap first pass is not built — the shipped route to the same question is the bootstrapped sup-test — and neither is a test for the *number* of regimes (Carrasco-Hu-Ploberger 2014)
