...
| Named observable trigger for system dynamics (spread, inflation) | `threshold_var`, linearity first via `threshold_var_test`; GIRFs still hand-rolled | Interpretable threshold; complete law of motion for counterfactuals |
...
- `tsecon.threshold_var`, `tsecon.threshold_var_test` — the two-regime TVAR of the first section: the concentrated log-determinant scan over $(\tau, d)$, and the robust score-form sup-Wald linearity test p-valued by Hansen's fixed-regressor wild bootstrap (seeded and parallel, so the p-value is bit-identical at any thread count). Regime-dependent GIRFs are deliberately *not* returned — the fit hands you two complete VARs and the threshold, and the simulation engine that would turn those into honest GIRFs is the roadmap item below
...
- **The GIRF engine** ([docs/roadmap/04-multivariate.md](../roadmap/04-multivariate.md), Tier 3): the shared Koop-Pesaran-Potter simulation engine — antithetic variates, common random numbers, reproducible parallelism, and the bootstrap-over-refits bands — contributed to the library's foundations for reuse by every nonlinear model, and with it the regime-dependent GIRFs that `threshold_var` declines to fake today. This is the single largest gap between what this chapter teaches and what it can hand you
