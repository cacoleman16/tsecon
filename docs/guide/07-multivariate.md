...
### Conditional forecasts: "if the policy rate stays on hold, what happens to output?"

The unconditional forecast answers "what does the model expect?". Policy institutions mostly ask a different question: *what does the model expect for output if inflation follows this path and the policy rate stays on hold for two quarters?* That is a **conditional forecast** — some cells of the future path are pinned, the rest are forecast given them — and it has a closed form (Doan, Litterman, and Sims 1984; Waggoner and Zha 1999). Stack the future innovations $u = (u_{T+1}', \dots, u_{T+H}')'$; every future cell is the unconditional forecast plus a known linear combination of $u$ (the MA coefficients $\Phi_h$ again), so pinning $m$ cells is a set of $m$ linear constraints $Bu = r$ on $u$. Under Gaussian innovations the conditional path is the unconditional path plus the response to $u^* = \Sigma B'(B \Sigma B')^{-1} r$ — the **minimum-norm** sequence of future shocks that delivers the conditions, which is also $E[u \mid Bu = r]$ — and the conditional covariance is the unconditional one minus what the conditions explain. It is exactly what a Kalman smoother returns when the free future cells are set missing (Bańbura, Giannone, and Lenza 2015), which is how `tsecon` validates it: against statsmodels' `VARMAX` smoother, at 1e-8, on every fixture case.
...
np.array(cf["point"])           # the conditional path; pinned cells hold their values exactly
np.array(cf["se"])              # 0 at pinned cells, smaller than cf["unconditional_se"] elsewhere
np.array(cf["shocks"])          # the implied future innovations the scenario needs
cf["mahalanobis_pvalue"]        # how hard the model is pushed: chi2(n_constrained) tail
```

The scenario pins five cells of this chapter's own system (columns: demand growth, output growth, policy rate). Output growth is never pinned, yet its standard error falls at every horizon — most at $h = 2$ and $h = 3$, to about 90% of the unconditional one — because output loads on *both* pinned series in $A_1$ (0.3 on demand, $-0.1$ on the rate), so the pinned paths carry information about it through the dynamics — and because later pins inform earlier innovations too. Conditioning tightens *every* free cell that shares innovations or dynamics with a pinned one, including horizons *before* the pinned one: the smoother runs backwards.

Read `mahalanobis_pvalue` before you read the path. It is the tail probability of the squared norm of the implied shocks under the model's own forecast distribution: a scenario the model would produce on its own scores near one, a scenario that needs a string of large same-signed innovations scores near zero — and the conditional forecast then describes a world the model finds implausible, not a likely one. Two further honest caveats: the intervals are innovation-uncertainty-only like `var_forecast`'s (coefficients treated as known), and `orth_shocks` — the Waggoner-Zha structural version of the implied shocks — depends on the column ordering through the Cholesky factor, whereas the path and its covariance do not.

### Residual diagnostics and lag order in one call each

Before any of this, the residuals have to be white. `tsecon.var_diagnostics(data, lags=2, nlags=10)` returns the multivariate Portmanteau statistic in both forms — `portmanteau` (Hosking's $Q_h$) and the small-sample `portmanteau_adjusted` (Lütkepohl 2005, section 4.4.3), each $\chi^2(k^2(h - p))$ so `nlags` must exceed `lags` — the multivariate Jarque-Bera test with its `skewness` and `kurtosis` components (the residuals orthogonalized by the Cholesky factor of their covariance, so the components depend on the column order as orthogonalized IRFs do), and the stability `roots` with the `is_stable` verdict. Report the adjusted Portmanteau at macro sample sizes: in `tsecon`'s own seeded Monte Carlo (Gaussian VAR(1), $k = 2$, $T = 200$, `nlags=8`, 1000 replications) the unadjusted statistic rejects a true null 4.6% of the time and the adjusted one 5.4% at a nominal 5% — both within Monte-Carlo error here, with the unadjusted undersize growing as `nlags` becomes a larger fraction of $T$. And choose the lag order on a common sample with `tsecon.var_select_order(data, max_lags=8)`, which fits every candidate after dropping the same `max_lags` presample rows so the AIC/BIC/HQIC/FPE columns are comparable — reading `aic` off `var_fit` at different `lags` compares fits on different samples and can flip the ranking.
...
> **⚠ Common mistake.** Using standard ADF p-values in step two. The residuals are not raw data — OLS *chose* $\hat\beta$ to make them as stationary-looking as possible, so the test statistic is biased toward rejection and needs stricter (more negative) critical values that depend on how many regressors were fitted (MacKinnon 2010 response surfaces; roughly $-3.34$ at 5% for one regressor plus constant, versus $-1.94$ for the raw no-constant ADF). Run step two on the *spurious* pair above and the standard machinery reports statistic $-3.13$ with $p = 0.002$ — a false detection of cointegration that the correct Engle-Granger critical value ($-3.34$) refuses. The $-15.8$ in the genuine example clears any threshold; borderline values are exactly where the correction decides. You do not have to assemble the correction by hand: `tsecon.engle_granger(data, trend="c")` runs both steps and returns `stat`, a MacKinnon `pvalue`, and the `crit` surface for the right number of regressors (statsmodels-exact at 1e-10 on the statistic), and `tsecon.phillips_ouliaris` gives the semiparametric $Z_t$/$Z_a$ alternative, which trades the residual-ADF lag choice for a Bartlett long-run-variance `bandwidth` (Newey-West rule by default).

Super-consistency is about the *point* estimate; the standard errors of that step-one regression are another matter. Its limit distribution is not centred and not normal — serial correlation in the equilibrium error, and correlation between that error and the regressor's innovations, leave a second-order bias and a nuisance-parameter limit — so the OLS t-statistics are simply wrong, and no HAC correction fixes them. Three classical estimators remove the nuisance terms and give a mixed-normal estimator whose t-statistics *are* standard normal under the null: `tsecon.fmols(y, x, trend="c")` (Phillips-Hansen 1990 fully modified OLS, which corrects the regressand for endogeneity and subtracts the serial-correlation bias using the kernel long-run covariance of the residual system), `tsecon.dols(y, x)` (Stock-Watson 1993 dynamic OLS, which augments the regression with leads and lags of $\Delta x_t$ chosen by BIC), and `tsecon.ccr(y, x)` (Park 1992 canonical cointegrating regression, which transforms the data instead). On the drunk-and-dog pair above all three agree with OLS to two decimals because the innovations are independent; on Wooldridge's T-bill-on-inflation data they move the slope from OLS's 0.64 to 0.74–0.85 and hand you a standard error of about 0.16 that means something. Each returns `ols_params` / `ols_se` beside its own `params` / `se` so the correction is visible, and each is `arch`-exact at 1e-10 (the model card lists the seeded Monte Carlo for size and super-consistency). They *condition* on cointegration — run `engle_granger` first.
...
Both objects in this section ship. `tsecon.factor_model` extracts the principal-component factors and answers the *how many* question (Bai-Ng and Ahn-Horenstein), and `tsecon.favar` fits the two-step Bernanke-Boivin-Eliasz FAVAR including the slow/fast rotation — the two sections at the end of this chapter work through both. The ragged-edge nowcasting member, `tsecon.dfm_nowcast` (Doz-Giannone-Reichlin two-step, plus a `method="mle"` route) with its `tsecon.dfm_news` revision decomposition, belongs to Chapter 11, which takes over from here. What is still [module-spec](../roadmap/04-multivariate.md) material is the wider DFM facade: mixed frequencies, arbitrary interior missing-data patterns, and EM estimation.
...
- **Pointwise bands are the wrong object anyway.** Readers interpret an IRF band as covering the whole path, but pointwise bands cover one horizon at a time. Montiel Olea and Plagborg-Møller (2019) supply sup-t *simultaneous* bands, and this is now literally a one-flag fix: `band="sup-t"`. It corrects the multiplicity exactly; it does not repair a standard error that was already short, which is why the measured sup-t rate lands at 85.2% rather than 90%.
...
- **Spillovers and networks.** Diebold and Yilmaz (2012, 2014) turned the generalized FEVD into connectedness measures now ubiquitous in empirical finance — with a hidden trap that `tsecon.connectedness` makes explicit rather than hiding: GFEVD rows do not sum to one, and competing normalizations silently change the numbers, so the function states which one it applies (the standard 2012 row-normalization). The last section of this chapter works it through.
...
| "If inflation follows this path, what happens to output?" | `var_conditional_forecast` | The Doan-Litterman-Sims / Waggoner-Zha closed form; read `mahalanobis_pvalue` first to see how hard the model is pushed |
| Are the residuals white and Gaussian, is the system stable, which lag order? | `var_diagnostics`, `var_select_order` | Adjusted Portmanteau, multivariate Jarque-Bera with components, stability roots; common-sample AIC/BIC/HQIC/FPE |
...
| I(1) series theory says move together | `engle_granger` (or `phillips_ouliaris`) | Superconsistent $\beta$, direct test of the equilibrium, MacKinnon p-values already applied; simplest credible check for one relationship |
| One equilibrium relation, and you need standard errors on its coefficients | `fmols` (or `dols` / `ccr`) | The static OLS is superconsistent but its t-statistics are invalid; the fully modified / dynamic / canonical corrections make them standard normal — after `engle_granger` has confirmed cointegration |
| Several I(1) series, unknown number of equilibria | `johansen`, then `vecm` at the selected rank | System estimation of the rank and all cointegrating vectors, plus adjustment speeds — match `deterministic=` to the case the rank test assumed |
...
| Hundreds of series, one underlying cycle | `factor_model` (how many, and the factors), then `favar` if you need one identified shock | A few factors capture the comovement; `favar` maps the shock back to every series in the panel |
| Same panel, but read in real time off a ragged edge | `dfm_nowcast`/`dfm_news` (Chapter 11) | Kalman machinery handles missing data and ragged edges, and attributes each revision to the release that caused it |
...
- `tsecon.var_conditional_forecast(data, conditions, lags=2, trend="c", steps=None, alpha=0.05)` — hard-path conditional forecasts (Doan-Litterman-Sims / Waggoner-Zha): the conditional `point` path, per-horizon `cov`/`se`, the implied `shocks`/`orth_shocks`, and a `mahalanobis_pvalue` plausibility check; validated against the closed form and statsmodels' `VARMAX` smoother
- `tsecon.var_diagnostics(data, lags=2, trend="c", nlags=10)` — Portmanteau (unadjusted and adjusted), multivariate Jarque-Bera with skewness/kurtosis components, stability roots
- `tsecon.var_select_order(data, max_lags=8, trend="c")` — common-sample AIC/BIC/HQIC/FPE lag-order selection
...
- `tsecon.vecm(data, k_ar_diff=2, coint_rank=1, deterministic="n", seasons=0)` — ML VECM estimation: `alpha`, `beta`, `det_coef_coint`, `gamma`, `det_coef`, `sigma_u`, `llf` (statsmodels-exact). `deterministic` takes all nine statsmodels cases: `"n"` (no deterministic terms, the default), `"co"` (unrestricted constant — the case `johansen` assumes; use it when the rank came from `johansen`), the restricted `"ci"`/`"li"` (constant/trend *inside* the cointegration relation — their coefficients widen the cointegrating matrix and come back as `det_coef_coint`), `"lo"`, and the combinations `"colo"`/`"coli"`/`"cilo"`/`"cili"`; `seasons=`/`first_season=` add centered seasonal dummies
- `tsecon.engle_granger(data, trend="c", autolag="aic")` — the two-step residual cointegration test with MacKinnon p-values and critical values already applied (column 0 is the regressand; do not add your own constant), and `tsecon.phillips_ouliaris(y, x, trend="c", test_type="Zt")` for the semiparametric $Z_t$/$Z_a$ alternative
- `tsecon.fmols(y, x, trend="c", kernel="bartlett", bandwidth=None)`, `tsecon.dols(y, x, lags=None, leads=None, ic="bic")`, `tsecon.ccr(y, x)` — Phillips-Hansen fully modified OLS, Stock-Watson dynamic OLS and Park canonical cointegrating regression of one cointegrating vector with valid (mixed-normal) inference: `params`/`se`/`tvalues`/`pvalues` on the regressors and deterministics, the long-run covariance pieces (`omega`/`lambda`/`sigma`), the `bandwidth` used (automatic by `bandwidth_rule="newey-west"` or `"andrews"`), DOLS's selected `lags`/`leads`, and `ols_params`/`ols_se` for comparison (`arch`-exact at 1e-10)
- `tsecon.var_irf_bands(data, ...)` — the banded companion to `var_irf`: Lütkepohl delta-method or residual/Kilian-bias-corrected bootstrap bands, and `band="sup-t"`/`"sidak"`/`"bonferroni"` for simultaneous bands over a `band_scope` you choose and it reports back
- `tsecon.connectedness(data, lags=2, horizon=10, trend="c")` — the Diebold-Yilmaz spillover table off the row-normalized generalized FEVD (last section of this chapter)
- `tsecon.factor_model(panel, n_factors=2, kmax=8)` — PCA factors, `eigenvalues`, the Bai-Ng `icp1`/`icp2`/`pcp1`/`pcp2` counts and the Ahn-Horenstein `er`/`er_ratios`
- `tsecon.favar(panel, policy, n_factors=2, lags=2, horizon=16, slow_indices=None)` — the two-step Bernanke-Boivin-Eliasz FAVAR, returning `irf_panel` ($N \times (H+1)$) and `irf_policy`
- `tsecon.historical_decomposition(...)` and `tsecon.structural_fevd(...)` — per-(time, variable, shock) contributions and a FEVD for any structural impact matrix, not just Cholesky (Chapter 8 uses both)
- Supporting cast used in this chapter: `check_stationarity`, `adf`, `kpss`, `ols`, `ljung_box`, `long_run_variance`

**Built in Rust, awaiting Python bindings** (in the `tsecon-var` crate): full multi-step forecast MSE matrices (`forecast_cov`) and the joint cross-horizon forecast-error covariance (`forecast_error_cov_joint`). The common-sample lag-order selection and the stability accessors that used to sit here are bound as of 0.10.0 (`var_select_order`, `var_diagnostics`).

**Roadmap** ([Module 04 — Multivariate Models](../roadmap/04-multivariate.md)): MacKinnon-Haug-Michelis p-values and Bartlett small-sample correction for the shipped Johansen tests, plus VECM restriction testing; Toda-Yamamoto causality; the remaining bootstrap IRF schemes (Gonçalves-Kilian wild and Brüggemann-Jentsch-Trenkler moving-block, beyond the shipped residual and Kilian-bias-corrected ones); generalized *IRFs* as a standalone call (the generalized FEVD already ships inside `connectedness`); the wider DFM facade — mixed frequencies, interior missing data, EM; VARX/VARMA; Markov-switching and smooth-transition VARs (`threshold_var`/`threshold_var_test` and the threshold VECM `threshold_vecm`/`hansen_seo_test` ship — Chapter 13 — and `star` is univariate, not a system STVAR); global VARs (the mean-group panel VAR ships as `mean_group_var`, Chapter 14).

## Three worked out in full: connectedness, factors, and FAVAR

`tsecon.connectedness`, `tsecon.factor_model`, and `tsecon.favar` each ship with Python bindings and a golden fixture, and each rewards more than a one-line entry in the list above. The three sections that follow are the working chapter for them — the intuition, the estimator, the call, and the traps.
