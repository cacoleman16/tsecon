...
Ridge is a two-line closed form, so the demo below writes it out in numpy — the point is to watch rolling-origin tuning pick $\lambda$, not to hide the estimator. Truth here is an AR(2); the design offers 24 lags against roughly 100 usable observations:
...
You do not have to keep the hand-rolled solver. `tsecon.ridge(X, y, alpha=lam_star)` returns the same coefficient vector — it is the same closed form, matching the `ridge` above to $7 \times 10^{-17}$ on this design, against scikit-learn's `Ridge` objective (add your own intercept column if you want one). The plain `tsecon.lasso` is its L1 twin. Swap it in and the loop above is unchanged; the folds are the part that has to stay honest, not the linear algebra.
...
The grouped variant — where all lags of one variable enter or leave together — ships as `tsecon.group_lasso` (0.8.0): one integer group label per column, a penalty `alpha` on the same $1/2n$ scale as `lasso`, and `l1_ratio > 0` to mix in the sparse-group term. Every fit returns its own subgradient KKT residual (`kkt_violation`), so `converged` is a certificate for this convex problem rather than an iteration count.

```python
groups = np.arange(X.shape[1]) // 4                       # four consecutive lags share a label
grp = tsecon.group_lasso(X, target, groups, alpha=0.05)   # a variable enters/leaves as a unit
print("active groups:", grp["active_groups"], " KKT residual:", f"{grp['kkt_violation']:.1e}")
```
...
For econometricians, **componentwise L2 boosting** (Bühlmann & Yu 2003; applied to recessions by Ng 2014) deserves a special mention: each boosting step fits a single predictor-lag by least squares and adds a small fraction of it to the model. Read sequentially, it is an automated ARDL-building procedure — a variable selector that econometricians can audit line by line — and it is deterministic and cheap; it ships as `tsecon.boosting`, with Bühlmann's (2006) corrected-AIC rule choosing the stopping step. Simpler still, bagging a pretest rule (estimate, keep what is significant, average over bootstrap replicates — Inoue & Kilian 2008) captures much of the ensemble benefit with none of the machinery, and is a good pedagogical bridge from OLS to ensembles.

Two dependence-specific cautions govern all tree methods on time series. First, the resampling inside a forest assumes exchangeable observations; on serial data it has to draw blocks, not rows. Second — see the mistake box. Both ship natively since 0.8.0: `tsecon.regression_tree` is CART with scikit-learn's split conventions (golden-pinned to `DecisionTreeRegressor`), and `tsecon.random_forest` grows those trees on iid, moving-block or stationary row resamples (`bootstrap="block"` / `"stationary"` with a `block_length`), returning out-of-bag predictions, Meinshausen quantile forests (`quantiles=`), and impurity or grouped block-permutation importance — the time-series-aware pieces the module design asked for. Gradient-boosted trees (XGBoost, LightGBM) stay *wrapped* behind optional adapters rather than reimplemented. A third-party learner still slots into the same discipline: `tsecon.cv_splits` for the folds, `tsecon.bootstrap_indices` / `tsecon.optimal_block_length` for block resampling, and `tsecon.backtest`, which takes a plain Python callable as the forecaster.
...
tsecon's policy follows from Chapter 0's scope discipline: **wrap, don't own**. Model adapters live in an optional companion package, because the model zoo churns annually. What the design keeps in core is the piece that does not churn: a *contamination-aware benchmark harness* — DM and related tests wired in, evaluation restricted to post-training-cutoff windows, real-time vintages respected, and flags raised when an evaluation series plausibly sits in a model's training corpus. That harness is a roadmap item, not a function you can call. What ships is its load-bearing half, and it is enough to run the audit by hand: point `tsecon.backtest` at the pretrained model through a Python callable, restrict the origins to the post-cutoff window yourself, and settle it with `tsecon.dm_test` against an AR benchmark.
...
These defaults — block permutation, lag-grouped attribution, ALE first — are what the roadmap's interpretation toolkit specifies. The first two ship inside the forest: `tsecon.random_forest(importance="block_permutation", importance_groups=..., permutation_block=...)` permutes contiguous blocks and scores all lags of a variable as one unit. The ml-trees model card measures where the grouping helps (per-lag permutation diluting a variable's importance across collinear lags) and where block permutation does *not* deliver: a persistent irrelevant predictor's inflated importance survives it in a row-wise forest. Shapley, partial-dependence and ALE functions are not in the Python API, so for those this section is a checklist for whatever tool you reach for.
...
- **Conformal prediction for time series** — distribution-free prediction intervals via EnbPI (Xu & Xie 2021) and adaptive conformal inference (Gibbs & Candès 2021) — loses its exchangeability foundation under dependence; the adaptive variants restore marginal coverage empirically. This one is no longer frontier-only: it ships as `tsecon.conformal_forecast` and `tsecon.conformal_backtest` (split, EnbPI, ACI). On the split and ACI routes `base=` accepts a plain Python callable, so it really does wrap any forecaster from this chapter, a scikit-learn model included; EnbPI is the exception — it trains its own AR ensemble and takes no callable base.
...
|---|---|---|
| Dozens-to-hundreds of correlated predictors, dense weak signals | `ridge`, or `factor_model` factors in a small regression | Macro panels are dense and factor-driven (Giannone-Lenza-Primiceri); smooth shrinkage and factors exploit exactly that structure |
| Genuinely sparse signal suspected, selection wanted | `lasso_path` (BIC-tuned via `bic_best`), `adaptive_lasso`, or `elastic_net` | Oracle-type selection under sparsity; elastic net stabilizes collinear lag choices |
| Whole variables should enter or leave (all their lags) | Group or sparse-group LASSO — genuinely absent; the honest stand-in is `lasso_path` plus your own judgment about lag blocks | Selection at the economic unit; per-lag selection is noise |
| Hundreds of series, need the standard benchmark | `factor_model`, then a small OLS readout on the factors | The canonical data-rich baseline every paper must beat; the factor section above runs exactly this |
...
| Tuning *any* hyperparameter above | `cv_splits(n, scheme="expanding")`, or `scheme="purged_kfold"` with `purge`/`embargo` when the K-times-cheaper budget matters | Random K-fold flatters every model under dependence |
...
**Available now in Python** — everything this chapter's runnable code called, plus the neighbours it points at: the leakage-safe splitter `cv_splits` (`"expanding"`, `"rolling"`, `"purged_kfold"` with purge and embargo) — the shipped replacement for this chapter's hand-rolled `rolling_origin_splits` / `purged_kfold_splits`, which are kept above only to show the bookkeeping; the penalized solvers — `ridge`, `lasso`, `elastic_net`, the glmnet-convention `lasso_path` with AIC/BIC selection, and `adaptive_lasso`; the diffusion-index machinery — `factor_model` (PCA factors plus the Bai-Ng `icp1`/`icp2`/`pcp1`/`pcp2` and Ahn-Horenstein `er` counts); the honest-evaluation spine — `accuracy` (RMSE/MAE/MASE and friends), `dm_test` (with the Harvey-Leybourne-Newbold correction), `theta_forecast` as a benchmark, and `backtest`, which takes a plain Python callable as the forecaster so anything in this chapter can be graded on a rolling origin; the distribution-free interval layer — `conformal_forecast` / `conformal_backtest` (split, EnbPI, ACI), whose `base=` also takes a Python callable; the dependence-aware resampling stack — `bootstrap_indices`, `optimal_block_length`, `philox_uniforms`; and the inference machinery ML methods lean on — `ols(se_type="hac")` and `long_run_variance`. The 0.8.0 wave added the module's Tier-2 surface: kernel methods (`kernel_ridge`; `kernel_regression` with a leave-block-out bandwidth selector), structured penalties and post-selection inference (`group_lasso`, `post_lasso`, `pds_lasso`), native trees and forests with block resampling and grouped block-permutation importance (`regression_tree`, `random_forest`), L1 trend filtering and componentwise boosting (`l1_trend_filter`, `boosting`), and the neural pair (`mlp_regression`, `echo_state_network`) — each graded on its model card at the validation it earned.

**Built in Rust awaiting bindings:** nothing from this module — every estimator written in Rust for it is bound to Python. The foundations-layer dependencies the later tiers will call are live in the crates: the Philox RNG, the block-bootstrap engine, the HAC/long-run-variance module (including fixed-b machinery), and the optimizer suite.

**Roadmap:** the full module design is in [docs/roadmap/10-machine-learning.md](../roadmap/10-machine-learning.md). Tier 1 has shipped — the TS-CV suite as `cv_splits`, the glmnet-convention path solvers as `lasso_path`/`elastic_net`/`ridge`/`lasso`/`adaptive_lasso`, and the diffusion-index factor extraction as `factor_model` — leaving the leakage-safe *pipeline protocol* (fit-transform-inside-the-fold, enforced mechanically) as the Tier-1 remainder, alongside the `factor_forecast` supervised-readout facade; Tier 2 shipped in 0.8.0 (the kernel, structured-penalty, tree, trend-filter, boosting and neural surfaces listed above). Still absent: GBT adapters, Shapley and ALE interpretation (block permutation and lag grouping ship inside `random_forest`), DML with block cross-fitting, the GLP sparsity diagnostic, the desparsified LASSO under dependence, the macroeconomic random forest, and the contamination-aware benchmark harness. Deep-learning and foundation-model adapters live in a companion package by scope ruling — wrap, don't own.
