...
Two dependence-specific cautions govern all tree methods on time series. First, the resampling inside a forest assumes exchangeable observations; on serial data it has to draw blocks, not rows. Second — see the mistake box. No tree learner ships in tsecon today: the module design (Tier 2) calls for a native random forest, needed for time-series-aware resampling and for the macroeconomic random forest below, with XGBoost and LightGBM *wrapped* behind optional adapters rather than reimplemented. Meanwhile you can already put scikit-learn's forest under honest discipline with the pieces that do ship: `tsecon.cv_splits` for the folds, `tsecon.bootstrap_indices` / `tsecon.optimal_block_length` for block resampling, and `tsecon.backtest`, which takes a plain Python callable as the forecaster.
...
These defaults — block permutation, lag-grouped attribution, ALE first — are what the roadmap's interpretation toolkit specifies. None of it is callable yet: no importance, Shapley, PD or ALE function is in the Python API today, so for now this section is a checklist for whatever tool you reach for, not a description of a shipped module.
