...
This chapter is the guide's tour of that territory — and it maps directly onto the library module that tsecon considers its headline differentiator, because no maintained Python package covers modern SVAR identification today. The check is concrete: statsmodels 0.15 ships `SVAR`, which does short-run A/B parameter restrictions and nothing else — no sign restrictions, no zero-plus-sign sampler, no proxy/external-instrument scheme, no narrative or robust-Bayes layer.
...
**On real data.** The [Blanchard-Quah replication](../examples/replication-blanchard-quah.md) runs `long_run_svar` with the paper's own design — US output growth demeaned around the 1974Q1 productivity break, detrended unemployment, eight lags — on the bundled 1959–2009 `macrodata` (a design replication at figure resolution: the paper's 1948–1987 GNP vintage is unreachable, so it pins shapes, not digits) and recovers the published shapes: a demand disturbance's effect on output peaks after three quarters (+1.10) and is gone by ten years (−0.004 at h = 40), the supply effect settles at a permanent +0.48, unemployment mirrors output, and demand accounts for about 90% of output's forecast-error variance a year out. The page also shows what the scheme hinges on: drop the break in mean growth and the long-run restriction reads the post-1973 slowdown as one enormous supply disturbance, collapsing demand's share of output variance from 97% to about 40% — the Faust-Leeper fragility above, made concrete. Its bootstrap bands and historical decomposition are built in the script from `var_fit`, `bootstrap_indices` and `long_run_svar`, since neither ships natively for long-run schemes yet.
...
The unit-effect normalization sets the policy rate's own impact response to exactly 1; the instrument is strong (F ≈ 475) despite covering only 380 of 500 periods — the `NaN` window is dropped from the moments automatically — and output falls on impact and stays below baseline: the contractionary-policy pattern, identified from one column with nothing assumed about the rest of the system. **This is a point estimate.** For inference, `proxy_svar_bands` gives the Jentsch-Lunsford moving-block bootstrap and `proxy_ar_sets` the weak-instrument-robust sets — see the [model card](../reference/model-cards/structural-identification.md#proxy_svar_bands-moving-block-bootstrap-bands-for-the-proxy-svar). The fluent `svar(...).identify_proxy(...)` report card remains a Module 06 roadmap item.
...
**Statistical identification, stress-tested.** Two members of this family are behind you already: two-regime Rigobon as `hetero_svar`, and non-Gaussianity as `nongaussian_svar` — mutually independent non-Gaussian shocks identify $B$ up to permutation and scale by the ICA theorem, failing if more than one shock is Gaussian, which is why the fit hands back `shock_kurtosis` for you to read before believing it. What is still frontier is the rest: Markov-switching variances, smooth-transition and GARCH covariances, and stochastic-volatility identification (Bertsche and Braun 2022; Lewis 2021). The honest open problem is that these methods rest on strong independence assumptions that are themselves economic claims (Montiel Olea, Plagborg-Møller, and Qian 2022); Drautzburg and Wright (2023) show how to relax independence into bounds. The library's documentation obligation, stated in the spec, is to teach when *not* to trust statistical identification.
...
| A high-frequency surprise series or other measured proxy | Proxy SVAR (`proxy_svar`), with `proxy_svar_bands` for moving-block bands and `proxy_ar_sets` for weak-IV-robust sets | Weakest assumptions on the rest of the system; check the first-stage F, and prefer the robust sets when it is modest |
