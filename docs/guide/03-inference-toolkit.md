...
Read the OLS column top to bottom: a test that is meant to reject 5% of the time creeps to 13% at $\rho = 0.99$ and to 24% at an exact unit root — roughly one "significant predictor" in four is a phantom, and it is *precisely* the persistent predictors (dividend yields, valuation ratios) where the distortion is worst. The IVX column holds its size across the entire ladder, including $\rho = 1$. The library's [Monte Carlo suite](../examples/monte-carlo.md#1-ivx-predictive-regressions-hold-their-size-at-a-unit-root) runs this at larger scale and pins the exact-unit-root numbers at **27.8% for OLS versus 5.3% for IVX** — over five times the nominal rate, dissolved. For a *panel* of competing predictors, `ivx_test` gives a single joint test. Its chi-square(`k`) Wald mode (`joint="chi2"`) has the same uniformity over *persistence* — but not over the *number* of predictors: at the same corner (ρ = 1, endogeneity −0.9, n = 250) its measured size at the default instrument tuning is 0.05 / 0.10 / 0.17 / 0.28 at k = 1/3/5/8, and growing n does not close it. That measured failure is why `ivx_test` now **defaults to `joint="bonferroni"`** (since 0.5; the default was `"chi2"` through 0.4.0) — the per-predictor scalar tests combined at level/k, whose measured size stays at or below nominal at every k (`alpha=0.5` under `joint="chi2"` is the weaker chi-square-shaped mitigation: it restores convergence in n but still measures ≈ 0.13 at k = 8, n = 250); both functions, the Stambaugh view, and the measured size and power tables are documented in the [predictive-regressions model card](../reference/model-cards/predictive-regressions.md).
...
- `tsecon.ols(y, X, se_type=..., maxlags=..., use_correction=...)` — OLS with the full ladder: `"nonrobust"`, `"hc0"`, `"hc1"`, `"hc2"`, `"hc3"`, `"hac"` (Bartlett kernel; `maxlags=None` applies the Newey–West rule of thumb). Matches statsmodels `cov_type="HAC"` at 1e-10 *when `use_correction` is matched* — the defaults differ deliberately (tsecon `use_correction=True`, the finite-sample $n/(n-k)$ scaling; statsmodels defaults it off), so at each library's own defaults the HAC `bse` differ by exactly $\sqrt{n/(n-k)}$; pass `use_correction=False` to reproduce a default statsmodels call — and `HC2`/`HC3` at 3e-15. An observation whose leverage is numerically 1 makes HC2/HC3 undefined — tsecon raises rather than returning a near-infinite standard error. `X` is used as-is — add your own constant column.
