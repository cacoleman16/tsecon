...
- **Hierarchical priors as the default.** Giannone, Lenza, and Primiceri (2015) ended the fixed-$\lambda$ era; the follow-on "prior on the long run" (Giannone, Lenza, and Primiceri 2019) disciplines long-horizon behavior through beliefs about great ratios rather than ad hoc dummy observations. The first of the pair is no longer frontier — it ships as `bvar_hierarchical`, the ML-II/MAP-II selection this chapter ran above; the fully sampled $\lambda$ posterior and the prior on the long run remain roadmap items with the authors' replication files as validation gates.
- **Correctness as a differentiator.** The field's most-cited samplers shipped wrong: Primiceri (2005) (corrected by Del Negro and Primiceri 2015) and Carriero-Clark-Marcellino (2019) (corrigendum: Carriero, Chan, Clark, and Marcellino 2022) — and uncorrected code still circulates. tsecon implements only corrected samplers, and the correctness evidence is specific rather than sweeping: the FFBS simulation smoother carries a Geweke (2004) "getting it right" joint-distribution test in the Rust suite, and the SSVS Gibbs sampler behind `bvar_ssvs` is graded by seeded Monte-Carlo support recovery, bitwise seed reproducibility, and multi-chain convergence. Simulation-based calibration (Talts et al. 2018) across every sampler is the roadmap's bar, not today's.
...
- **Global-local shrinkage.** The horseshoe prior (Carvalho, Polson, and Scott 2010) and Normal-Gamma/Dirichlet-Laplace families shrink adaptively — hard on noise, gently on signal — and win several BVAR forecasting horse races; the roadmap puts them behind one "shrinkage family" interface so users can horse-race priors. None of that family is callable yet — the shipped sparse-VAR route is the spike-and-slab `bvar_ssvs`.
...
- **Honest uncertainty statements.** Joint credible bands for IRFs (Inoue and Kilian 2022) fix the pointwise-band miscoverage above and are still roadmap; the rank-normalized diagnostics (Vehtari et al. 2021) are not — they ship in Python as `mcmc_diagnostics`, matching ArviZ and R `posterior` numerically.
...
|---|---|---|
| Forecasting VAR, 3–10 variables, typical macro sample | `bvar_fit` — the conjugate Minnesota/NIW-BVAR, tightness compared via its `log_marginal_likelihood` | Closed form — fast, no convergence worries; shrinkage cures overparameterization |
...
| 20–130 variables | Large conjugate BVAR, shrinkage tightened with dimension — **roadmap**; `bvar_fit` is the same prior at the scales above | Bańbura et al. (2010): beats factor models; conjugacy keeps it feasible |
| Prior must differ freely across equations | Independent Normal-Wishart + Gibbs — **roadmap** | The Kronecker restriction of the conjugate form is the price of closed forms |
| Suspected drift in dynamics (policy regimes, structural change) | TVP-BVAR via FFBS, with shrinkage on state variances — **roadmap** (the FFBS smoother exists in the Rust crate, unbound); `markov_switching_ar` is the shipped discrete-regime alternative | Random-walk coefficients; shrinkage prevents hallucinated time variation |
| Volatility clustering, crisis samples, fan charts | BVAR-SV (order-invariant variant if available) — **roadmap**; univariate SV-style volatility ships as `gas_volatility`/`dcs_local_level` (Chapter 6) | Constant-variance bands mislead in both calm and turbulent periods |
| Comparing lag lengths, priors, variable sets | `bvar_fit`'s `log_marginal_likelihood` differenced across specifications (Bayes factors); BMA is **roadmap** | Evidence integrates out parameters; automatic Occam penalty |
| Any MCMC output, before believing it | `mcmc_diagnostics` on 4 chains — rank-normalized split $\widehat{R}$ < 1.01, bulk *and* tail ESS | Single chains and trace-plot eyeballing miss stuck and slow-mixing samplers |
| Reporting IRF uncertainty from a posterior | `bvar_irf_draws`, then your own per-horizon quantiles, labeled pointwise vs. joint | Credible bands answer the probability question people actually ask |
