...
| Panel LP over long horizons in a short panel | `panel_lp(..., jackknife=True)` | Nickell bias grows like $O(h/T)$; split-panel correction tames it — but at short $T$ the corrected estimate is noisier than its unchanged `se` admits (coverage 0.880 → 0.804 at $T=60$), so read the band as indicative |
