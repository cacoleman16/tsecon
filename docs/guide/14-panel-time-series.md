...
- `tsecon.panel_lp(outcome, shock, horizon=8, n_lag_controls=2, se_type="driscoll_kraay", bandwidth=4.0, cumulative=False, jackknife=False, bias_correction="none")` — panel local projection of a common `shock` (length $T$) on an $N \times T$ outcome with entity fixed effects; `cumulative` returns the summed multiplier, `jackknife` applies the Dhaene-Jochmans half-panel correction (points only), and `bias_correction="spj"` the Mei-Sheng-Shi split-panel jackknife (points and SEs). Returns `irf`, `se`, `nobs`, plus the stamped `se_type`/`cumulative`/`jackknife`/`bias_correction`.
