...
Distinguishing them is the job of the HEGY test (Hylleberg, Engle, Granger and Yoo, 1990), which tests for unit roots at the zero frequency and at each seasonal frequency separately, and of the Canova-Hansen test (Canova and Hansen, 1995), which — KPSS-style — takes stable seasonality as its null. Both are on the [Module 01 roadmap](../roadmap/01-diagnostics-exploration.md), alongside the QS test (the X-13 residual-seasonality workhorse) and the Friedman and Kruskal-Wallis rank tests; none of the four has a credible Python implementation today, and HEGY in particular is a headline gap the library aims to fill.

What does ship is the decision rule practitioners actually reach for. `tsecon.stl` (and `tsecon.mstl` for multiple periods) decomposes the series; `tsecon.seasonal_strength` reads the Wang-Smith-Hyndman strength off that decomposition; and `tsecon.nsdiffs` turns the strength into a verdict — the `forecast::nsdiffs(test="seas")` rule, differencing at the seasonal lag while the strength stays at or above 0.64:

```python
ns = tsecon.nsdiffs(ts, period=12)
ns["d"], ns["stop"]                      # 1 'WeakSeasonality' — one seasonal difference, then done
ns["steps"][0]["seasonal_strength"]      # 0.96 — the strength that triggered it
tsecon.seasonal_strength(ts, period=12)["seasonal_strength"]   # 0.96, the same number standalone
```

That is a *strength threshold*, not a hypothesis test — it answers "is the seasonality strong enough that differencing helps?", not "is there a seasonal unit root?" The second question is what HEGY and Canova-Hansen are for, and it is the one still open.
...
**Back to the trap: the unit-root test that knows about breaks.** The whole reason this ladder sits inside a diagnostics chapter is that `check_stationarity` misread `y_brk` as I(1). `zivot_andrews` is the direct repair — it searches the break date *inside* the unit-root regression, so the alternative it tests against is "stationary around one broken deterministic component" rather than "stationary around a constant":

```python
za = tsecon.zivot_andrews(y_brk)          # regression="c": an intercept shift
za["stat"], za["pvalue"]                  # (-6.299, 0.0005) — the unit root is rejected
za["crit"]["1%"]                          # -5.276: past the 1% value, not a marginal call
za["break_index"]                         # 99 — the first break, found by the test itself
```

Same series, same data, opposite verdict: the plain quadrant said **UnitRoot**, and a test allowed one break says stationary with $p = 0.0005$. Two caveats keep this honest. `zivot_andrews` admits exactly *one* break, and this series has two — it finds the first and absorbs the second into the residual, which is why the ladder above (and `bai_perron` in particular) is still the tool for counting them. And its p-values interpolate a simulated table, so read at most two decimals into them.

> **⚠ Common mistake.** Concluding "unit root" from ADF without ever asking about breaks. The two-break example above did exactly that on purpose: `check_stationarity` returned **UnitRoot** on a series that is white noise around a broken mean, and only `bai_perron` told the truth. The **Conflict** cell of the quadrant (ADF rejects, KPSS rejects) is often the same signature — and so is a stubborn non-rejection on a series with an obvious level shift. Whenever the quadrant surprises you, run `sup_f_test` before you difference — or run the unit-root test that already knows about breaks: `tsecon.zivot_andrews` puts one endogenous break inside the null-hypothesis machinery, so a stationary series around a broken mean is no longer forced to read as I(1). Break-robust unit-root tests exist because the plain ADF verdict is unreliable in the presence of breaks; the two-break Lee-Strazicich (2003) extension is still a roadmap item.
...
**Better unit-root tests.** Plain ADF is no longer the state of the art, and the two standard upgrades are now calls. DF-GLS (Elliott, Rothenberg and Stock, 1996) detrends by GLS before testing and gains substantial power near the null — `tsecon.dfgls`, matching `arch.unitroot.DFGLS` to 1e-10. The Ng-Perron M-tests with the MAIC lag-selection rule (Ng and Perron, 2001) fix the severe size distortions that negative MA errors inflict on ADF and Phillips-Perron — `tsecon.ng_perron`, returning MZa, MZt, MSB and MPT against the published critical values (there are no p-values, because no response surface for the M-tests has ever been published; compare each statistic to its own table). What remains on the roadmap is the layer above them: facing uncertainty about trends and initial conditions, the union-of-rejections strategies of Harvey, Leybourne and Taylor (2009, 2012) combine OLS- and GLS-detrended tests with size-corrected joint critical values, and that is where the `check_stationarity` workflow is designed to escalate next.

**Breaks everywhere.** The frontier treats breaks and unit roots jointly: Zivot and Andrews (1992) and Lee and Strazicich (2003) allow breaks within the unit-root test itself — the single-break Zivot-Andrews case ships as `tsecon.zivot_andrews` and is run above; Lee-Strazicich's two-break version, and Carrion-i-Silvestre, Kim and Perron (2009), who push to multiple breaks under both null and alternative with GLS detrending, do not. On the pure break side, modern changepoint methods (PELT, wild binary segmentation) trade econometric inference for speed on very long series — the roadmap positions them explicitly as *detection*, with the shipped `bai_perron` owning *inference*. The main inference gaps that remain: the heterogeneity-robust break-date intervals, and breaks in variance rather than mean.
...
**Long memory.** Between I(0) and I(1) lies fractional integration: ACFs that decay like a power law rather than geometrically. The semiparametric estimators of the memory parameter are thin on the ground in Python — neither statsmodels 0.15 nor `arch` 8.0 estimates $d$ — and tsecon ships two of them as `tsecon.long_memory_d`: log-periodogram regression (Geweke and Porter-Hudak, 1983) and local Whittle (Robinson, 1995), each with a bandwidth-honest standard error rather than only the textbook asymptotic one. What is still open is the harder half: the exact local Whittle of Shimotsu and Phillips (2005), and Qu's (2011) test distinguishing true long memory from level shifts masquerading as it.
...
| Monthly/quarterly data, calendar rhythm suspected | `acf` at lags $s, 2s$ to detect; `seasonal_strength`/`nsdiffs` to decide; `stl`/`mstl` to remove | Seasonal spikes are unmistakable; the strength rule decides whether to difference |
| Seasonal *unit root* vs. a fixed December effect | QS/HEGY/Canova-Hansen (roadmap) | The strength rule answers "strong enough to difference?", not "is the pattern itself drifting?" |
...
| Inconclusive quadrant, small sample | `dfgls`, then `ng_perron` if a negative MA root is plausible | GLS detrending buys power near the null; MAIC fixes the size distortion ADF and PP both suffer |
| Unit-root verdict on a series with a visible level shift | `zivot_andrews` (one break inside the test) | Perron's trap: a broken-mean stationary series tests as I(1) under plain ADF |
| How many differences, as a rule rather than a judgement call | `ndiffs` (seasonal: `nsdiffs`) | The Hyndman-Khandakar sequential rule, with the per-order test evidence returned |
| Estimating the memory parameter of a long-memory series | `long_memory_d` (`"gph"` or `"local_whittle"`) | ACFs between I(0) and I(1) need $d$, not a differencing decision |
...
- `dfgls(y, regression="c"|"ct", lags=None, method="aic"|"bic"|"t-stat")` → `{"statistic", "p_value", "used_lag", "nobs", "crit", "trend"}` — GLS-detrended ADF (Elliott-Rothenberg-Stock 1996); statistic and selected lag match `arch.unitroot.DFGLS` to < 1e-10
- `ng_perron(y, regression="c"|"ct", lags=None|"maic"|int)` → `{"mza", "mzt", "msb", "mpt", "used_lag", "nobs", "s2_ar", "crit", "trend"}` — the four M-tests with MAIC lag selection; **critical values only, no p-values** (none are published), and all four reject when *small*
- `zivot_andrews(y, regression="c"|"t"|"ct", trim=0.15, autolag="aic")` → `{"stat", "pvalue", "crit", "break_index", "lags", "nobs", ...}` — unit root against stationarity around one endogenous break; matches statsmodels' `zivot_andrews` (statistic 1e-10, break and lag exact)
- `ndiffs(y, test="kpss"|"adf"|"pp", alpha=0.05, max_d=2)` and `nsdiffs(y, period, max_d=1)` → `{"d", "stop", "steps", "interpretation"}` — the Hyndman-Khandakar differencing advisors, each returning the evidence at every order it tried
- `stl(y, period, ...)`, `mstl(y, periods=[...])`, `seasonal_strength(y, period)` — STL and multi-seasonal STL (statsmodels-matched elementwise at 1e-8) and the Wang-Smith-Hyndman strengths that `nsdiffs` reads
- `long_memory_d(x, method="gph"|"local_whittle", m=None)` → `{"d", "se", "se_asymptotic", "m", ...}` — the two semiparametric memory-parameter estimators; build intervals from `se`, which is the bandwidth-honest one
...
**Roadmap** ([docs/roadmap/01-diagnostics-exploration.md](../roadmap/01-diagnostics-exploration.md)): Breusch-Godfrey, HEGY and Canova-Hansen seasonal unit roots, QS/Friedman seasonality tests, the heterogeneity-robust Bai-Perron break-date intervals and variance breaks, Lee-Strazicich two-break unit roots, the union-of-rejections unit-root strategies, multitaper and cross-spectral phase spectral estimation, exact local Whittle and Qu's memory-versus-breaks test, BDS, GSADF bubble tests, a fitted-model-aware residual battery, and X-13 seasonal adjustment.
