...
- `ng_perron(y, trend="c"|"ct", lags=None|"maic"|int)` → `{"mza", "mzt", "msb", "mpt", "used_lag", "nobs", "s2_ar", "crit", "trend"}` — the four M-tests with MAIC lag selection; **critical values only, no p-values** (none are published), and all four reject when *small*
