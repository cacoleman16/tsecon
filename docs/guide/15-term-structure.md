...
The correction $A(\tau)/\tau$ is a function of the *factor volatilities* and the decay — not of the fitted factors — and it captures a convexity, or Jensen's-inequality, effect that the reduced-form model simply omits. Here is the intuition. A bond's price is a *convex* function of the yields that discount it — $e^{-\tau y}$ curves upward as $y$ falls — and Jensen's inequality says expectations of a convex function of a volatile state sit *above* the function of the expectation: $\mathbb{E}[f(X)] \geq f(\mathbb{E}[X])$ for convex $f$. Rate volatility therefore pushes the expected bond price above the price at the expected rate path, and a higher price is a *lower* yield. The more volatile the factors and the longer the horizon over which that volatility compounds, the larger the wedge. So the adjustment is **non-positive** — it can only pull yields down, never up — and it **deepens with maturity**, because volatility has more time to compound out at the long end. At the short end there is almost nothing to correct; the gap opens up precisely where pricing cares about it. And it **vanishes as $\Sigma \to 0$**: with no factor volatility there is no convexity wedge, so $A(\tau)/\tau \to 0$ and AFNS collapses back onto plain Nelson-Siegel exactly. AFNS *nests* the reduced-form model, adding the no-arbitrage correction as a volatility-scaled perturbation on top of it.
...
**Term-premium decomposition.** A long yield is (roughly) the average expected short rate over the bond's life *plus* a term premium for bearing duration risk. Splitting the two is a central question for policy, and the regression-based decomposition of Adrian, Crump and Moench (2013) — curve factors, a factor VAR, and excess-return pricing regressions — has become the practitioner standard, precisely because it lives one step beyond the factor extraction this chapter teaches. It is now callable as `tsecon.acm_term_premium` (see the [term-structure model card](../reference/model-cards/term-structure.md)), validated against the NY Fed's published ACM series.

**The canonical affine model, by maximum likelihood.** The ACM decomposition estimates the affine model by regression; the *likelihood-based* version is where the term-structure literature actually lives, and for a decade its practical obstacle was rotation indeterminacy — a Gaussian affine model with $N$ latent factors can be rotated, scaled and translated without changing a single yield, so unconstrained maximum likelihood wanders on a flat ridge. Joslin, Singleton and Zhu (2011) removed the obstacle. Their **canonical form** puts the risk-neutral dynamics into a shape with only $N + 1$ free parameters — the ordered Q-eigenvalues $\lambda^Q_1 \ge \dots \ge \lambda^Q_N$ (the Jordan form; equal neighbours form a block) and one drift $k^Q_\infty$, with the short rate $r_t = \iota' X_t$ — and then rotates the latent state onto $N$ observed yield *portfolios* $P_t = W y_t$ (the first $N$ principal components by default) that the model prices without error. Two things follow. The rotation is now a normalization rather than a choice, so any basis of the same portfolio space gives the same fit. And the likelihood *factors* into a term for the portfolio dynamics and a term for the cross-sectional pricing errors: the physical VAR(1) of $P_t$ appears only in the first, whose maximizer is OLS whatever the innovation covariance is — so it is concentrated out exactly, and the numerical search runs only over $\lambda^Q$ and the Cholesky factor of $\Sigma_P$ ($k^Q_\infty$ and $\sigma_e$ profile out in closed form too). That is what `tsecon.jsz_fit` implements; `tsecon.jsz_loadings` evaluates the canonical Riccati recursions at parameters of your choosing. The connections to this chapter are exact rather than loose. Feed `jsz_loadings` the eigenvalue pattern $\lambda^Q = (1, e^{-\lambda\Delta}, e^{-\lambda\Delta})$ and its yield loadings span the Nelson-Siegel loadings *exactly* (a $3 \times 3$ rotation, residual at machine precision), while its convexity intercept converges — at first order in the period length $\Delta$, because discrete-time JSZ replaces CDR's integral by a Riemann sum — to the `afns_adjustment` term above (the gap is 9.8e-6 at monthly resolution, 6.1e-7 at $\Delta = 1/192$): AFNS *is* the JSZ model with two eigenvalues tied, and `jsz_fit` lets the data decide whether they are. And the risk-neutral yields and the term premium are defined exactly as `acm_term_premium` defines them — the same recursion re-run under the P-measure VAR — so the two premia are comparable on the same panel; on GSW 1990-2007 (JSZ's own window) the fit reports $\lambda^Q = (0.9965, 0.9624, 0.9092)$, a 2.7bp pricing error, and a mean 10-year premium of 2.30pp. Two traps, both documented on the [model card](../reference/model-cards/term-structure.md): the likelihood is *flat in the market prices of risk* (the Q parameters are pinned by the cross-section to four digits; the P-VAR — and hence $\lambda_0, \lambda_1$ and the premium's *level* — is not, which is why the OLS standard errors are returned and no standard errors are claimed for the Q parameters), and the level factor is nearly a unit root under Q ($\lambda^Q_1 = 0.9965$, a 16-year half-life), which the recursions handle exactly but the long-run Q mean does not survive.
...
| Split a long yield into expected short rates and a *term premium* | `acm_term_premium` on a monthly yield panel | The Adrian-Crump-Moench three-step regressions price the time series of returns — the practitioner-standard decomposition behind the NY Fed's published series |
| The *maximum-likelihood* affine model: Q-eigenvalues, a likelihood, no-arbitrage loadings, the same premium | `jsz_fit` on the same panel | The JSZ canonical form kills the rotation indeterminacy and concentrates the P-VAR out by OLS; nests AFNS at a tied eigenvalue pair |
...
- `tsecon.acm_term_premium(yields, maturities, n_factors=5, periods_per_year=12.0)` — the Adrian-Crump-Moench (2013) regression-based term premium on a $T \times M$ panel of *annualized decimal* zero-coupon yields at *integer-period* maturities containing 1; returns the `fitted` / `risk_neutral` / `term_premium` decomposition with the full model (factors, VAR, prices of risk, recursions) and diagnostics — validated against an independent NumPy pipeline at 1e-8 and against the NY Fed's published 10-year ACM premium (correlation 0.985, RMSE 0.31pp on 1961-2014); see the [model card](../reference/model-cards/term-structure.md)
- `tsecon.jsz_fit(yields, maturities, n_factors=3, periods_per_year=12.0, w=None, n_starts=5, seed=None)` — the Joslin-Singleton-Zhu (2011) canonical Gaussian affine model by maximum likelihood on the same kind of panel (the one-period maturity is not required); returns the Q parameters `lambda_q`, `k_inf_q`, the innovation covariance `sigma` and pricing error `sigma_e`, the concentrated P-VAR `mu_p`/`phi_p` with OLS standard errors, the ACM-unit prices of risk, loadings in the portfolio and canonical rotations, the same `fitted` / `risk_neutral` / `term_premium` decomposition, `llf`, and `converged`; the recursions are a documented-formula golden at 1e-12, the P-VAR a statsmodels golden at 1e-9, the MLE a SciPy cross-optimizer target, and the AFNS special case is pinned to `afns_adjustment` (see the [model card](../reference/model-cards/term-structure.md))
- `tsecon.jsz_loadings(lambda_q, k_inf_q, sigma_x, maturities, periods_per_year=1.0)` — the canonical Riccati recursions at given parameters (Jordan blocks at exactly tied eigenvalues), returning `a_x`, `b_x`, `k0_q`, `k1_q`

All of them take **NumPy arrays** — pass `np.array(...)`, not bare Python lists (except the integer `maturities` of `acm_term_premium`, `jsz_fit` and `jsz_loadings`, which may be lists). The factor extraction is a cross-sectional OLS (chapter 2's regression machinery); the optimal-$\lambda$ and Svensson fits add a small nonlinear search over the decay(s); the dynamic model layers per-factor AR(1) estimation (chapter 4) on top; `afns_adjustment` is a deterministic evaluation of the CDR closed form given volatilities you supply; `acm_term_premium` is nothing but OLS end to end — the whole affine model estimated without a single call to an optimizer; and `jsz_fit` is the maximum-likelihood affine model with everything that *can* be concentrated out concentrated out, so its optimizer sees only the Q-eigenvalues and a Cholesky factor.

**Roadmap** — the further extensions the frontier section named are specified but not yet callable: the one-step **state-space / Kalman-filter** DNS (for honest forecast bands), the general **correlated-factor AFNS** and JSZ models with *all* yields measured with error (the Kalman-filter JSZ likelihood), the **DNS-macro** joint state-space VAR (for curve-macro feedback), and **shadow-rate** models for the lower bound.
...
- **Joslin, S., K. J. Singleton and H. Zhu (2011), "A New Perspective on Gaussian Dynamic Term Structure Models," *Review of Financial Studies*.** The canonical form that removes the rotation indeterminacy of affine models and factors the likelihood so the physical VAR concentrates out by OLS — the estimator behind `jsz_fit`.
