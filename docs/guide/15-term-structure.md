...
All five take **NumPy arrays** — pass `np.array(...)`, not bare Python lists (except `acm_term_premium`'s integer `maturities`, which may be a list). The factor extraction is a cross-sectional OLS (chapter 2's regression machinery); the optimal-$\lambda$ and Svensson fits add a small nonlinear search over the decay(s); the dynamic model layers per-factor AR(1) estimation (chapter 4) on top; `afns_adjustment` is a deterministic evaluation of the CDR closed form given volatilities you supply; and `acm_term_premium` is nothing but OLS end to end — the whole affine model estimated without a single call to an optimizer.
