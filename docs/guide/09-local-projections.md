...
The prerequisite is the joint cross-horizon covariance matrix, which requires estimating all horizons as one stacked system rather than $H+1$ unrelated regressions. That machinery now exists: `lp_irf_cov` in the `tsecon-lp` crate builds the full cross-horizon matrix from the Frisch-Waugh-Lovell influence function, and `band="sup-t"` on `lp` uses it. It is a Rust-side object, not a Python callable — you reach it through `band=`, which also reports `cov_se_max_rel_diff`, the largest relative gap between that covariance's own $\sqrt{\mathrm{diag}}$ and the reported `se`. It is first-class internally precisely so that sup-t bands, path Wald tests ("is the IRF zero at all horizons?"), and multiplier delta methods all fall out of it; the latter two remain roadmap. Computing per-horizon standard errors and pretending horizons are independent produces bands that are wrong in both width and shape.
...
The `first_stage_f` ladder — reported by both estimators — is the Montiel Olea-Pflueger effective F, horizon by horizon; a value below the rule-of-thumb 10 is the signal to switch from a point estimate to a weak-instrument-robust Anderson-Rubin set. sup-t simultaneous bands now ship for `lp` and `smooth_lp` via `band="sup-t"`. `lp_iv`, `lp_multiplier` and `lp_state` have no cross-horizon covariance yet, so they get Šidák/Bonferroni only and refuse sup-t with an error rather than approximating one. Anderson-Rubin sets for LP-IV remain on the [roadmap](../roadmap/07-local-projections.md).
...
> **⚠ Common mistake — trusting `flp`'s per-score standard errors as if the scores were observed.** The scores are `functional_pca` *estimates*, i.e. generated regressors, and the HAC sandwich in `flp` conditions on them as if they were data — the same two-step hazard flagged for FAVAR factors in [chapter 7](07-multivariate.md) and for the dynamic Nelson-Siegel two-step in [chapter 15](15-term-structure.md). Measured: the per-element `se` runs about **0.66 of the true sampling sd**, flat over a 16× range of $T$ (so it is inconsistency, not a small sample — coverage ≈ 0.80 against a nominal 0.95), and it worsens as the curve panel's signal-to-noise falls; on the model card's own worked example the second score's `se` is one-fifth of the truth. Scores you *supply* from outside are real data and the `se` is then correct. The route that survives estimation of the basis is the one this section already reports: `flp_scenario`'s $w'\beta$ contrast is algebraically invariant to rotations of the estimated eigenfunctions (rotate $\hat\Phi$ and the weights rotate to match), so quote scenario responses and their `se`, not raw per-score betas — the [functional-shocks model card](../reference/model-cards/functional-shocks.md) carries the measured numbers.
...
The LP literature is one of the most active in econometrics; here is the current edge. Most of it sits in the roadmap module's upper tiers — but not all, as the last item records.
...
| Staggered policy adoption in a panel | `lp_did` (clean controls built in; read `n_switchers` per horizon) | Avoids TWFE negative-weight pathologies, and the negative horizons are the pre-trend check |
| A common shock across a short panel, long horizons | `panel_lp(bias_correction="spj")` | Nickell bias grows like $O(h/T)$ in panel LP; the split-panel jackknife corrects the points *and* the SEs |
...
**Roadmap:** the dedicated module ([docs/roadmap/07-local-projections.md](../roadmap/07-local-projections.md)) hardens what ships today and owns what cannot reasonably be hand-rolled: path Wald tests over the joint cross-horizon covariance (the covariance and sup-t bands themselves now ship), Anderson-Rubin weak-instrument sets for LP-IV (the proxy-SVAR analogue ships as `proxy_ar_sets`; the LP-IV one does not), wild and block bootstrap schemes that are actually valid for LP, the smooth-transition (logistic) state-dependent variant with endogeneity diagnostics, Bayesian and GLS LP, and LP-VAR dual reporting. LP-DiD has left this list — it ships as `lp_did`. Its validation bar: reproduce the Ramey-Zubairy multipliers and the `lpirfs`/Stata reference numbers to three-plus decimals.
