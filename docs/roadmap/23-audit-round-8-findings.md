...
- **`lp_did` vs a live R/fixest run** — no R in this environment; the
  ...
  reference at 1e-10. *(Made since — repository audit, September 2026: the
  validation matrix's `lp_did` row is a reference-run golden from an R/fixest
  execution of the authors' example code, `fixtures/generate_lpdid_fixtures.R`,
  fixest compiled from the CRAN mirror; the NumPy reimplementation agrees at
  5.3e-15.)*
