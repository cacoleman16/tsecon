...
| Functional local projections (functional shocks) | The shock is a *curve*, not a scalar: a policy announcement shifts the entire yield curve, and the LP estimates the response to that whole functional shift — the right tool for unconventional monetary policy, where "the" policy shock has a level, slope, and curvature all at once. | Research-grade | Inoue & Rossi's functional-shock approach: parameterize the announcement-day shift of the discount curve (e.g., Nelson-Siegel level/slope/curvature or a functional principal-component basis), then run LPs of outcomes on the projection coefficients jointly — the IRF is a functional: response per point of the original curve. Implementation notes: the basis choice is an identification choice (document it loudly); joint inference across basis coefficients needs the stacked-LP covariance (sup-t bands over the curve dimension as well as horizon); the covariate contract's aligned-ingestion machinery handles the curve data; visualization needs response *surfaces* (horizon × maturity heatmaps — Module 13). Inoue & Rossi (2021, Journal of International Economics; 2019 REStat). Pairs with the E2 term-structure module for curve construction. Gate: reproduce the Inoue-Rossi exchange-rate response surfaces from their replication files. |
