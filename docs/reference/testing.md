...
| Python binding tests | **454 passed** in 5.1 s | `.venv/bin/python -m pytest bindings/python/tests -q` |
| Crates | 40, **every one** with a `tests/` directory | |
| Golden fixtures | 45 JSON files, produced by 26 generator scripts | `fixtures/` |
| Public Python functions | 105 — **all 105** are called at least once in the binding suite | |
...
454 tests in 40 files. 37 of the 45 fixture JSONs are reloaded here and checked
...
- **Benchmarks compare 25 of 105 functions.** The parity gate covers the unit-root
