...
code on synthetic data, and the figure it produces. All figures are generated by
...
The roadmap-E methods — recession probability, survey expectations, long memory,
the CUSUM stability test, predictive regressions / IVX, the arbitrage-free
Nelson-Siegel adjustment, and the linear rational-expectations (DSGE-lite)
saddle path. All seven figures are generated by
[showcase_extensions.py](showcase_extensions.py):

```sh
.venv/bin/python docs/examples/showcase_extensions.py
```

### Recession probability from a leading spread

**Use case:** turn a leading indicator — classically the term spread — into a
calibrated probability of recession. A probit (or the Kauppi-Saikkonen dynamic
probit) maps the spread to `P(recession)`, the quantitative version of the
"inverted yield curve" warning.

```python
X = np.column_stack([np.ones(n), spread])
fit = tsecon.recession_probit(y, X, link="probit")   # y is the 0/1 indicator
fit["probabilities"]     # fitted P(recession); also params, pseudo_r2, ...
```

![Recession probit](img/ext-recession-probit.png)

The spread (top) falls ahead of each shaded downturn; the fitted probability
(bottom) climbs through 0.5 just before it, exactly as a leading indicator
should. See the [recession model card](../reference/model-cards/recession.md).

### Survey expectations: rigidity and disagreement

**Use case:** test whether professional forecasts are fully rational and measure
how much forecasters disagree. The Coibion-Gorodnichenko regression of the mean
forecast *error* on the mean *revision* has slope zero under full-information
rational expectations; a positive slope is the signature of information rigidity.

```python
cg = tsecon.cg_regression(errors, revisions)   # HAC-robust; slope, implied_rigidity
dis = tsecon.forecast_disagreement(panel)      # per-period std, IQR, quantiles
```

![Survey expectations](img/ext-survey.png)

Left: the CG regression — the dashed line is the rational-expectations null
(zero slope), the positive fit its sticky-information departure. Right:
cross-sectional disagreement widening through a turbulent stretch. See the
[survey expectations card](../reference/model-cards/expectations.md).

### Long memory: a hyperbolic ACF and the filter that removes it

**Use case:** some series (realized volatility, inflation, trading volume) decay
too slowly for an ARMA but are not unit-root either — they have *long memory*,
`0 < d < 0.5`. Estimate `d`, then fractionally difference to whiten.

```python
d_hat = tsecon.long_memory_d(x, method="local_whittle")["d"]   # Robinson 1995
x_white = tsecon.frac_diff(x, d_hat)                            # (1 - L)^d
```

![Long memory](img/ext-long-memory.png)

Left: the raw series' ACF decays hyperbolically — still 0.2 at lag 40. Right:
after `(1 − L)^d̂` the autocorrelation sits inside the 95% band; the estimate
covers the true `d = 0.40`. See the
[long-memory card](../reference/model-cards/long-memory.md).

### CUSUM: parameter stability over the sample

**Use case:** did the regression relationship hold constant, or did the
coefficients drift/break somewhere you did not pre-specify? The Brown-Durbin-Evans
CUSUM of recursive residuals is a random walk near zero under stable
coefficients and walks out of its 5% band when they shift.

```python
cs = tsecon.cusum_test(y, X)          # path, bound_upper, bound_lower, sigma
breach = np.any(cs["path"] > cs["bound_upper"]) or np.any(cs["path"] < cs["bound_lower"])
```

![CUSUM stability](img/ext-cusum.png)

The stable fit (blue) stays inside the band; the series with a mid-sample break
(red) crosses it decisively. See the
[specification-tests card](../reference/model-cards/specification-tests.md).

### Predictive regressions: IVX keeps its size near a unit root

**Use case:** regressing returns on a *persistent, endogenous* predictor (a
dividend yield, a valuation ratio) breaks the OLS t-test — it over-rejects the
harder the predictor's root approaches one. The IVX Wald test holds its size
uniformly, up to and including an exact unit root.

```python
fit = tsecon.predictive_regression(r, x)   # ols, stambaugh, and ivx views
fit["ivx"]["wald"], fit["ivx"]["pvalue"]   # the size-correct headline test
```

![IVX size](img/ext-ivx-size.png)

Under a true no-predictability null, the naive OLS t-test's rejection rate climbs
from 6% to 25% as `ρ → 1`, while the IVX Wald test stays near its nominal 5%. See
the [predictive-regressions card](../reference/model-cards/predictive-regressions.md).

### Arbitrage-free Nelson-Siegel: one extra term

**Use case:** a plain Nelson-Siegel curve fits yields well but is not
arbitrage-free. Christensen-Diebold-Rudebusch keep the three loadings unchanged
and add a single closed-form adjustment term `−A(τ)/τ` that restores no-arbitrage.

```python
adj = tsecon.afns_adjustment(maturities, sigma, decay=0.5)   # signed -A(tau)/tau
y_afns = y_ns + adj                                          # add to any NS fit
```

![AFNS](img/ext-afns.png)

Left: the adjustment is zero at the short end and deepens with maturity and
factor volatility. Right: added to the reduced-form curve it pulls the long end
down — the no-arbitrage convexity effect. See the
[AFNS card](../reference/model-cards/afns.md).

### DSGE-lite: the rational-expectations saddle path

**Use case:** solve a small linearized rational-expectations model to its policy
rule and check determinacy. The Blanchard-Kahn count — unstable roots vs jump
variables — decides whether the equilibrium is unique.

```python
sol = tsecon.dsge_solve(A, B, C, n_predetermined=1)   # g, p, q, eigenvalue_moduli, verdict
sol["verdict"]   # "unique stable solution (1 unstable eigenvalue(s) = 1 jump variable(s))"
```

![DSGE saddle path](img/ext-dsge-saddle.png)
