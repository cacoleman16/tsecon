...
# ==========================================================================
# Experiment 6 -- the simultaneous (sup-t) band
# ==========================================================================
SIM_N_SIM = 20_000
"""Gaussian draws behind each sup-t critical value in experiment 6.
...
def report_simultaneous(res):
    ...
    header(
        f"EXP 6  Simultaneous (sup-t) bands.  {res['dgp']}, n = {res['n']}, "
        f"R = {res['reps']}\n"
...
def _band_request_inert(y):
    ...
    also what licenses experiment 6 to read both arms off one call.
    """
...
def assertions(results, facts, reps):
    ...
    check(
        ...
        e6["simultaneous_nests_pointwise"],
        f"c >= z on all {len(SIM_METHODS)} routes and all {r6} replications",
    )
    jp6 = e6["joint_pointwise"]
    for m in SIM_METHODS:
        j, gain, loss = e6["joint"][m], e6["paired_gain"][m], e6["paired_loss"][m]
        ...
        check(
            ...
            loss == 0 and gain >= 9,
            f"{100 * j:.1f}% vs pointwise {100 * jp6:.1f}% over K={e6['n_cells']} "
            f"horizons; {gain} discordant replications gained, {loss} lost",
        )
    j_supt = e6["joint"]["sup-t"]
    check(
        "sup-t still does NOT reach nominal here, and the shortfall is inherited",
        j_supt < NOMINAL - 3.0 * mc_se(j_supt, r6),
        f"sup-t joint {100 * j_supt:.1f}% vs nominal {100 * NOMINAL:.0f}%; the same "
        f"band's marginal coverage at h={len(e6['truth']) - 1} is "
        f"{100 * e6['coverage_pointwise'][-1]:.1f}%",
    )
    check(
        "sup-t is tighter than both closed forms (it uses the cross-horizon dependence)",
        e6["critical_value"]["sup-t"] < e6["critical_value"]["sidak"]
        < e6["critical_value"]["bonferroni"],
        "  <  ".join(f"{m} {e6['critical_value'][m]:.4f}" for m in SIM_METHODS)
        + f"  (pointwise {e6['pointwise_critical_value']:.4f})",
    )
...
def findings(results, reps):
    ...
    lines.append(
        f"EXP6 n={e6['n']:<4} the SAME draws, K={e6['n_cells']} horizons: the pointwise band "
        f"contains the whole path in {100 * e6['joint_pointwise']:.1f}% of samples, the sup-t "
        f"band in {100 * e6['joint']['sup-t']:.1f}% (sidak {100 * e6['joint']['sidak']:.1f}%, "
        f"bonferroni {100 * e6['joint']['bonferroni']:.1f}%), nominal {100 * NOMINAL:.0f}%."
    )
    lines.append(
        f"EXP6 n={e6['n']:<4} sup-t does NOT reach nominal, and that is not multiplicity: the "
        f"same band covers {100 * e6['coverage_pointwise'][0]:.1f}% marginally at h=0 and "
        f"{100 * e6['coverage_pointwise'][hmax6]:.1f}% at h={hmax6}. What is left needs a better "
        f"standard error, not a bigger multiplier."
    )
    lines.append(
        f"EXP6 n={e6['n']:<4} multipliers at K={e6['n_cells']}, alpha={ALPHA:.2f}: pointwise "
        f"{e6['pointwise_critical_value']:.4f}, "
        + ", ".join(f"{m} {e6['critical_value'][m]:.4f}" for m in SIM_METHODS)
...
def run(quick=False, reps=None):
    ...
    results["exp6"] = exp_simultaneous(max(reps // 2, REPS_QUICK), horizon, n=ns_small[-1])
    report_simultaneous(results["exp6"])
