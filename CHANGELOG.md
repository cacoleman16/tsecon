...
The Tier-1 gaps that a forecasting library is judged on, closed in one wave:
Harvey's structural time-series models and the full ETS taxonomy for the
univariate side, the conditional forecast and the residual diagnostics every
VAR write-up needs, the model-set verdict that replaces pairwise forecast
comparisons, and the cointegrating regressions applied work actually uses.
Beside them the gallery gains its first Phase-2-gate paper, and a hygiene round
takes the panel estimators off balanced data, puts a budget on the allocations
that could kill the interpreter, and makes every refusal name the parameter it
is refusing. Thirteen new callables, 179 to 192; 44 crates; 1939 Rust and 2344
Python tests, all green.

Audit round 14 swept the thirteen new callables, the `mask=` parameter and the
Blanchard-Quah page before release (126 candidates, 77 refuted, 10 confirmed and
fixed with 34 pins, 0 severe).

Building the structural models turned up three defects in the shared
exact-diffuse Kalman filter, one of which inflated a log-likelihood by tens of
points whenever a state the observation never loads on held the diffuse period
open — exactly what a twelve-period trigonometric seasonal builds. The whole
workspace suite is green on the fix with no pinned number moved.

### Added

- `unobserved_components` — Harvey's structural time-series models by exact-diffuse maximum likelihood: every statsmodels level/trend specification, dummy and trigonometric seasonals, the damped stochastic cycle, regressors estimated jointly, and missing observations. Returns parameters with observed-information standard errors and per-parameter pile-up flags, log-likelihood, AIC/BIC on statsmodels' `k_params + k_diffuse` convention, filtered and smoothed states and components with variances, standardized residuals, and forecasts with variances. Pinned at 1e-8 against statsmodels `UnobservedComponents(use_exact_diffuse=True)` on 26 component combinations and three NaN-inserted series; the MLE pinned to the better of two optimizers on the same criterion. The Nile local level reproduces the Durbin-Koopman book values (15098.5 / 1469.18 against 15099 / 1469.1).
- `tvp_regression` — regression with random-walk coefficients on the same machinery, with the Shephard-Harvey/Stock-Watson pile-up check on every coefficient variance, filtered and smoothed coefficient paths with variances, and statsmodels `RecursiveLS` as the zero-state-variance limit.
- **ETS — the innovations state-space exponential-smoothing family** (`ets_fit`, `auto_ets`; new crate `tsecon-ets`). All 30 ETS(Error, Trend, Seasonal) models of Hyndman, Koehler, Snyder & Grose (2002) and Hyndman, Koehler, Ord & Snyder (2008) by maximum likelihood over the smoothing parameters and (by default) the initial states, with the Hyndman (2008 §2.6.1) heuristic initialisation, exact class-1 prediction intervals from the Table 6.1 closed forms and seeded simulated intervals for every other class. `auto_ets` runs R's `forecast::ets` candidate search (AICc by default) and returns the winner fitted together with the ranked candidate table. Validated at fixed parameters against statsmodels `ETSModel` at 1e-10 for the twenty models whose smoother is the innovations form (and against its `simulate` for all thirty), against an independent NumPy transcription of the published recursion at 1e-12 for all thirty, against `holtwinters.ExponentialSmoothing` for the heuristic initialisation, and match-or-beat against `ETSModel.fit()` on 21 maximum-likelihood cases under two optimizers (worst shortfall 2.67e-6 relative; better than statsmodels' boundary-stalled optimum by 0.89 on log-UKgas ETS(A,A,A)). The ten multiplicative-seasonal models differ from statsmodels by convention, not by error; the gap is measured (0.276–0.487 in log-likelihood on the airline series) and stated. The selection loop is graded by seeded Monte-Carlo recovery, as `auto_arima` is: 0.90–1.00 at T = 600.
- `var_conditional_forecast` — hard-path conditional VAR forecasts (Doan-Litterman-Sims / Waggoner-Zha closed form): conditional path, per-horizon covariance and intervals, the implied minimum-norm reduced-form and Cholesky-orthogonalised shocks, and a Mahalanobis plausibility p-value; `conditions` as a horizon×k nested list, NaN array or DataFrame with free cells. Two exact goldens: the closed form in NumPy (1e-10) and statsmodels' `VARMAX(...NaN future...).smooth(params)` Kalman-conditioning route (1e-8, the two agreeing at 1.8e-15); measured 95% conditional coverage 0.9470–0.9605.
- `var_diagnostics` — multivariate Portmanteau (unadjusted and adjusted), multivariate Jarque-Bera with skewness/kurtosis components, and the stability roots, matching statsmodels `test_whiteness` / `test_normality` / `roots` at 1e-10; measured size 4.60% / 5.40% / 4.20% at nominal 5%, power 300/300 against an omitted lag and against t(4) innovations.
- `var_select_order` — binds the existing common-sample AIC/BIC/HQIC/FPE lag-order selection (statsmodels `VAR.select_order` picks and tables at 1e-8).
- **Multiple forecast comparisons** in `tsecon-forecast`: `spa_test` (White's 2000 Reality Check and Hansen's 2005 test for Superior Predictive Ability, with the lower / consistent / upper re-centrings and their p-values), `model_confidence_set` (Hansen-Lunde-Nason 2011, the `T_R` and `T_max` statistics with the elimination order, per-step p-values and per-model MCS p-values) and `stepm_test` (Romano-Wolf 2005, which models beat the benchmark at a controlled family-wise error rate). All three consume a `T x m` loss table of the kind `backtest` produces, resample it with the library's stationary / circular-block / moving-block bootstraps, choose the block length by the Politis-White rule when `block_size=None`, and are bit-identical at any thread count. Pinned against `arch` 8.0.0 bit-for-bit on the reference's own resample indices, with the seeded path at Monte Carlo tolerance; size, power and MCS coverage measured by seeded Monte Carlo against arch's own rates on the same designs. The studentized default over-rejects at n = 200 (0.1225 at nominal 0.05, 0.0925 at n = 800) — Hansen's own construction, measured and documented in the card, the guide and the docstring.
- `fmols`, `dols`, `ccr` — Phillips-Hansen (1990) fully modified OLS, Stock-Watson (1993) dynamic OLS with AIC/BIC/HQIC lead-lag selection, and Park (1992) canonical cointegrating regression for a single cointegrating vector, with asymptotically valid (mixed-normal) inference, the long-run covariance pieces used, the bandwidth actually chosen (`"newey-west"` — `arch`'s rule — or `"andrews"`), and the plain OLS estimates beside them. `arch` 8.0-exact at 1e-10 across every trend, kernel and option (151 golden cases); super-consistency, the second-order OLS bias and t-statistic size measured by seeded Monte Carlo and quoted in the model card. Two documented deviations: the Andrews bandwidth rule (a closed form; `arch` has none) and CCR under `df_adjust` (the documented `T/(T−k)` scaling; `arch` 8.0 has an operator-precedence slip). The underdetermined default DOLS search `arch` runs silently on short samples is refused with the caps named.
- `tsecon-coint`: `long_run_covariance` / `automatic_bandwidth` (multivariate kernel long-run, one-sided and short-run covariances in `arch`'s window convention, on `tsecon-hac`'s kernels), `CointTrend`, `BandwidthRule`.
- **Blanchard-Quah (1989) replication page** — the first Phase-2-gate item: `long_run_svar` on the paper's design (1974Q1 mean break, detrended unemployment, VAR(8)) over the bundled statsmodels `macrodata` 1959–2009, a *design replication at figure-reading resolution* (the 1948–1987 GNP vintage is unreachable) that pins the published shapes — hump-shaped, vanishing demand effect on output (+1.10 at h = 3, −0.004 at h = 40), permanent supply effect (+0.48), mirror-image unemployment response, supply raising unemployment on impact, demand's 89–97% share of short-horizon output variance and 65–91% of unemployment's, recessions as demand — with seeded residual-bootstrap bands, a historical decomposition cross-checked against the library at 9e-15, a NumPy closed-form dual check on a statsmodels VAR fit (2e-13), a sensitivity table showing the design hinges on the mean break, and three house-style figures; guarded offline by `test_replication_blanchard_quah.py` on the committed `fixtures/macrodata_bq.csv`.
- Unbalanced panels: `panel_fe`, `panel_distributed_lag` and `panel_lp` take an observation mask (`mask=`, an N x T array of 0/1 or bool flags, 1 where the entity is observed in that period); cells outside the mask are ignored and may hold NaN. The within projections, the time effects (partialled out by the Frisch-Waugh route `PanelOLS` uses on unbalanced data), the effect counts and the cluster/Driscoll-Kraay score sums all run over the observed cells, and a lagged row enters only when every one of its lags is observed. Validated against `linearmodels` 7.0 `PanelOLS` at 1e-10 on the Arellano-Bond `EmplUK` panel (140 firms, 1976-1984, 1031 firm-years) and on seeded ragged panels, with `nobs`/`df_resid` exact; every balanced-panel result is pinned bit-identical to 0.9.0.
- `lp_did` and `panel_lp`'s half-panel jackknives accept `mask=` and refuse an unbalanced panel with the reason, rather than silently assuming balance.
- EnbPI and ACI conformal forecasts are now cross-checked against `mapie` 1.5.0's `TimeSeriesRegressor` (`fixtures/conformal_mapie.json`): ACI exactly (bounds 1e-12 relative, misses and the `alpha_t` trajectory exact), EnbPI statistically (coverage, width and ensemble centres). Closes the validation-matrix follow-up VM-01. The cross-check found that the reference defaults to an asymmetric conformity score, which the earlier draft comparison had silently inherited.

### Changed

- `tsecon-coint` depends on `tsecon-hac` (one HAC owner); `CointError` gains `Hac` and `InvalidSpec` variants.
- The GIRF engines (`var_girf`, `threshold_var_girf`) size their working set up front against a fixed 2 GiB budget and refuse an oversized request as a `ValueError` naming `n_draws`, the horizon and the number of histories; the large buffers are allocated with `try_reserve`, so an allocator refusal is the same error rather than a process abort.
- No parameter default renders as `Ellipsis` in `inspect.signature` any more (11 did in 0.9.0): tuple, vector and enum defaults are `None` sentinels with the effective default documented, and `adf`/`zivot_andrews`/`engle_granger` show `autolag='aic'`.
- `unobserved_components`' default `cycle_period_bounds` upper period is the sample length, deliberately unlike statsmodels: under exact-diffuse initialization the cycle log-likelihood diverges as the frequency goes to zero (measured: 25 points over four decades) and a free optimizer reports a cycle of period 10⁶.
- The gallery now lists nine replication pages; the `long_run_svar` model card, the validation matrix and the identification guide chapter link the replication.

### Fixed

- **The univariate exact-diffuse filter could take the diffuse branch on cancellation roundoff and inflate the log-likelihood by tens of points.** It surfaced whenever a state the observation never loads on held the diffuse period open, which is exactly what `freq_seasonal=[12]` (statsmodels' default six harmonics) builds: `lltrend` + `freq_seasonal(period 4)` returned −198.83 against statsmodels' −253.81 and now agrees to 1e-8. `P_inf` is also zeroed when the diffuse period ends instead of being amplified for the rest of the sample, which had made a post-diffuse forecast report an infinite variance a hundred periods later. The whole workspace suite is green on the fix with no pinned number moved.
- Every refusal now names the offending parameter: the 83 cells of audit round 13's malformed-input sweep that did not are all fixed, in the Rust messages and in the three PyO3 boundary messages the coercion layer rebuilds, with the outcome of all 673 sweep cells otherwise unchanged.
- **Two guards counted bytes where the cost counts time.** `panel_distributed_lag` on an unbalanced panel with time effects partials them out through the projected time dummies, which is exact and cubic in T: 0.11 s at T = 400 and 38 s at T = 3200 (N = 6) against 3 ms unmasked or with `time_effects=False`. And `var_conditional_forecast`'s `steps` budget is on memory, so it admits a horizon the quadratic work turns into hours — `steps=200_000` was the one cap cell of 137 that did not finish in 60 s. Neither algorithm changed (the exact routes are what the goldens pin); both costs are now measured and stated on the docstring, the stub, `api.md` and the card, with the escapes (audit round 14, M1 + M2).
- Every refusal names its parameter again: the 34 cells of audit round 14's 1563-cell malformed-input sweep that did not are down to 2 — `ets_fit`'s `smoothing_params` / `initial_states` length messages and the sample-size message a huge `seasonal_periods` triggered, the SPA/MCS automatic block-length failure (which read as a sentence spliced into a requirement slot and blamed `block_size` rather than `losses`), `freq_seasonal_harmonics`, the `panel_lp` / `lp_did` window messages and the ragged-losses message. The residue is a cascade that names the constraint that actually failed, and `alpha=1e-300`, where `1 − alpha/2` rounds to 1 and the normal quantile refuses by its own symbol (round 14, L1 + L2).
- The coercion layer could not see a bad value nested inside a list argument: a string cell in `var_conditional_forecast(conditions=…)` raised "an argument of type str is of type str", naming nothing. The offender scan now walks three levels and names the cell by position, while a shallow list keeps round 13's wording (round 14, L3).
- `fmols` / `ccr`: `df_adjust` scales the covariance by `(T−1)/(T−1−p)` with `p` the estimated coefficients — regressors **and** deterministics — not `(T−1)/(T−1−k)` as four surfaces said, nor the card's `T/(T−k)`. At the default `trend="c"` with k = 2 the stated factor was 199/197 and the actual one 199/196 (round 14, L4).
- `fmols` / `ccr`: the docstring called `False` the default of `diff`; the default is the `None` sentinel, and `diff=False` passed explicitly raises at the default trend exactly as `diff=True` does (round 14, L5).
- `band_alpha` was validated only inside the band builder, so `band_alpha=nan` with `band=None` returned in silence on `lp`, `lp_iv`, `lp_multiplier`, `lp_state`, `panel_lp` and `smooth_lp`. It is now checked unconditionally: a call passing a `band_alpha` outside (0, 1) with no band used to return and now raises (round 14, L7).
- Guide chapter 14 advertised `panel_fe(outcome, regressors, se_type="cluster", bandwidth=4.0)` — a call that raises, because `bandwidth` is refused under any `se_type` but Driscoll-Kraay — and three of the four panel signature bullets still lacked the `mask=` the release added. The `unobserved_components` card example printed `np.float64(...)` wrappers rather than the line its expected-output comment claimed (round 14, L6 + L8).
- **The bootstrap working set was guarded by `try_reserve` alone, which is not portable.** Linux's overcommit heuristic refuses a multi-terabyte request outright, so `spa_test(..., reps=10**12)` raised a teaching error there; macOS hands the reservation out lazily and the kernel kills the process when the buffer is written. The macOS CI job died that way, reproducibly, inside the very test that asserts an impossible replication count is refused rather than allocated. Every bootstrap allocation in `spa_test`, `stepm_test` and `model_confidence_set` now checks an explicit 2^28-value (2 GiB) budget *before* the allocator is asked — the convention `var_girf` and the ETS simulated intervals already use — with `try_reserve` kept as the second line of defence, and the `reps x models` product that sized one buffer is now a checked multiply. Pinned on both sides at the budget boundary rather than at a count only some allocators reject.
- **Five inputs killed the interpreter instead of raising.** A mistyped `steps` in `var_conditional_forecast`, `max_lags` in `var_select_order`, or `horizon` / `n_sim` in the ETS simulated intervals reached the Rust allocator and aborted the process — uncatchable from Python, because all of them sit below the 2^48 pre-flight the coercion layer applies. Each is now refused by name before anything is allocated, with regression tests on both sides.
