...
/// The fixture case object with the given name.
pub fn find_case<'a>(fx: &'a Value, name: &str) -> &'a Value {
    fx["cases"]
        .as_array()
        .expect("cases array")
        .iter()
        .find(|c| c["name"].as_str() == Some(name))
        .unwrap_or_else(|| panic!("case {name} not in fixture"))
}

/// Values of a JSON object of numbers, in insertion order (the fixture is
/// written by Python with ordered dicts, and serde_json preserves order
/// only with the `preserve_order` feature — so read them by the arch
/// parameter names instead, in the order the spec defines).
pub fn params_by_name(case: &Value, key: &str, spec: &GarchSpec) -> Vec<f64> {
    let obj = &case[key];
    spec.param_names()
        .iter()
        .map(|n| {
            obj[n]
                .as_f64()
                .unwrap_or_else(|| panic!("{key}[{n}] missing"))
        })
        .collect()
}

/// The model specification for a fixture case name.
pub fn spec_for(name: &str) -> GarchSpec {
    let mean = if name.contains("const") {
        MeanSpec::Constant
    } else {
        MeanSpec::Zero
    };
    let vol = if name.starts_with("gjr") {
        VolSpec::Gjr { p: 1, o: 1, q: 1 }
    } else if name.starts_with("egarch") {
        VolSpec::Egarch { p: 1, o: 1, q: 1 }
    } else {
        VolSpec::Garch { p: 1, q: 1 }
    };
    let dist = if name.ends_with("_t") {
        DistSpec::StudentT
    } else {
        DistSpec::Normal
    };
    GarchSpec { mean, vol, dist }
}
