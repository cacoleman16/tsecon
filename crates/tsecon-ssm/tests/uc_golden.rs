...
//!    NaN-inserted series — pinned at [`TOL_FIXED`].
...
//!    fixture records both; on one case statsmodels' fit stops 4.5
//!    log-likelihood points short, which is why two are recorded) within
//!    [`TOL_MLE_LL`], and the parameters within [`TOL_MLE_PARAM`] where the
//!    optimum is interior; observed-information standard errors against
//!    statsmodels `cov_type="approx"` within [`TOL_SE`].
...
/// Observed-information standard errors vs statsmodels' complex-step
/// `cov_type="approx"` (relative).
...
fn spec_from_json(spec: &Value, x: &[Vec<f64>]) -> UcSpec {
    ...
    s.damped_cycle = spec.get("damped_cycle").and_then(|v| v.as_bool()).unwrap_or(false);
...
fn assert_component(c: &UcComponent, block: &Value, tol: f64, what: &str) {
    assert_series(&c.filtered, &as_f64_vec(&block["filtered"]), tol, &format!("{what}.filtered"));
    assert_series(
        &c.filtered_var,
    ...
    assert_series(&c.smoothed, &as_f64_vec(&block["smoothed"]), tol, &format!("{what}.smoothed"));
    assert_series(
        &c.smoothed_var,
        &as_f64_vec(&block["smoothed_var"]),
        tol,
        &format!("{what}.smoothed_var"),
    );
}

/// Every fixed-parameter quantity of a block against the fit.
fn check_fixed_block(fit: &UcFit, block: &Value, what: &str) {
    assert_eq!(fit.k_states, block["k_states"].as_u64().unwrap() as usize, "{what}: k_states");
    assert_eq!(
        fit.nobs_diffuse,
        block["nobs_diffuse"].as_u64().unwrap() as usize,
        "{what}: nobs_diffuse"
    );
    assert_eq!(fit.param_names, names(&block["param_names"]), "{what}: param_names");
    assert_rel_close(fit.loglik, block["loglike"].as_f64().unwrap(), TOL_FIXED, &format!("{what}: loglike"));
    assert_rel_close(fit.aic, block["aic"].as_f64().unwrap(), TOL_FIXED, &format!("{what}: aic"));
    assert_rel_close(fit.bic, block["bic"].as_f64().unwrap(), TOL_FIXED, &format!("{what}: bic"));
    assert_paths(&fit.filtered_state, &block["filtered_state"], TOL_FIXED, &format!("{what}: filtered_state"));
    assert_paths(
    ...
    assert_paths(&fit.smoothed_state, &block["smoothed_state"], TOL_FIXED, &format!("{what}: smoothed_state"));
    assert_paths(
    ...
    assert_series(&fit.fitted, &as_f64_vec(&block["fitted"]), TOL_FIXED, &format!("{what}: fitted"));
    assert_series(&fit.resid, &as_f64_vec(&block["resid"]), TOL_FIXED, &format!("{what}: resid"));
    // statsmodels writes 0.0 for the standardized residual inside the
    // diffuse period; this crate reports NaN there (no finite prediction
    // variance exists). Compare after the diffuse period only.
    let sr = as_f64_vec(&block["std_resid"]);
    for (t, (a, e)) in fit.std_resid.iter().zip(&sr).enumerate() {
        if t < fit.nobs_diffuse {
            assert!(a.is_nan(), "{what}: std_resid[{t}] inside the diffuse period must be NaN");
        } else if e.is_nan() {
            assert!(a.is_nan(), "{what}: std_resid[{t}] at a missing period must be NaN");
        } else {
            assert_rel_close(*a, *e, TOL_FIXED, &format!("{what}: std_resid[{t}]"));
        }
    }
    if let Some(fc) = block.get("forecast") {
        assert_series(&fit.forecast, &as_f64_vec(fc), TOL_FIXED, &format!("{what}: forecast"));
        assert_series(
            &fit.forecast_var,
    ...
    if let Some(comp) = block.get("components") {
        if let Some(b) = comp.get("level") {
            assert_component(fit.level.as_ref().expect("level"), b, TOL_FIXED, &format!("{what}: level"));
        }
        if let Some(b) = comp.get("trend") {
            assert_component(fit.slope.as_ref().expect("slope"), b, TOL_FIXED, &format!("{what}: slope"));
        }
        ...
        if let Some(b) = comp.get("cycle") {
            assert_component(fit.cycle.as_ref().expect("cycle"), b, TOL_FIXED, &format!("{what}: cycle"));
        }
        if let Some(bs) = comp.get("freq_seasonal") {
            ...
            for (i, (c, b)) in fit.freq_seasonal.iter().zip(bs).enumerate() {
                assert_component(c, b, TOL_FIXED, &format!("{what}: freq_seasonal[{i}]"));
            }
        }
    }
}
...
fn check_mle(fit: &UcFit, mle: &Value, what: &str) {
    ...
    assert!(ll_ref >= ll_sm && ll_ref >= ll_sc, "{what}: fixture picks the better optimum");
    assert!(
...
fn nile_local_level_fixed_at_durbin_koopman_values() {
    ...
    let fit = unobserved_components(&nile, &UcSpec::default(), &fixed_opts(block, 10, Vec::new())).unwrap();
...
fn nile_local_level_mle_reproduces_durbin_koopman_and_both_optimizers() {
    ...
    assert!((fit.params[0] - dk[0]).abs() < 1.0, "sigma2_eps {} vs DK {}", fit.params[0], dk[0]);
    assert!((fit.params[1] - dk[1]).abs() < 0.2, "sigma2_eta {} vs DK {}", fit.params[1], dk[1]);
...
fn fixed_parameter_component_combinations_match_statsmodels() {
    ...
    for case in cases {
        ...
        let fexog = if spec.exog.is_empty() { Vec::new() } else { xf.clone() };
...
fn missing_observations_fixed_parameters_match_statsmodels() {
    ...
    assert!(y[10].is_nan() && y[40].is_nan() && y[124].is_nan() && !y[125].is_nan());
    let x = columns(&fx["sim"]["x"]);
    let xf = columns(&fx["sim"]["x_forecast"]);
    for case in fx["missing"].as_array().unwrap() {
        ...
        let fit = unobserved_components(&y, &spec, &fixed_opts(case, 8, fexog))
            .unwrap_or_else(|e| panic!("{name}: {e}"));
        assert_eq!(fit.nobs_observed, y.iter().filter(|v| v.is_finite()).count());
...
fn check_tvp_fixed(fit: &tsecon_ssm::TvpFit, block: &Value, what: &str) {
    ...
    assert_rel_close(fit.aic, block["aic"].as_f64().unwrap(), TOL_FIXED, &format!("{what}: aic"));
    assert_rel_close(fit.bic, block["bic"].as_f64().unwrap(), TOL_FIXED, &format!("{what}: bic"));
    assert_eq!(fit.nobs_diffuse, block["nobs_diffuse"].as_u64().unwrap() as usize);
    assert_paths(&fit.beta_filtered, &block["beta_filtered"], TOL_FIXED, &format!("{what}: beta_filtered"));
    assert_paths(
    ...
    assert_paths(&fit.beta_smoothed, &block["beta_smoothed"], TOL_FIXED, &format!("{what}: beta_smoothed"));
    assert_paths(
    ...
    let sr = as_f64_vec(&block["std_resid"]);
    for (t, (a, e)) in fit.std_resid.iter().zip(&sr).enumerate() {
        if t < fit.nobs_diffuse || e.is_nan() {
...
fn tvp_fixed_parameters_match_the_statsmodels_transcription() {
    ...
    assert_eq!(fit_m.nobs_observed, ym.iter().filter(|v| v.is_finite()).count());
...
fn tvp_zero_state_variance_is_recursive_least_squares() {
    ...
    assert_rel_close(fit.loglik, rls["llf"].as_f64().unwrap(), TOL_FIXED, "rls llf");
    assert_paths(&fit.beta_filtered, &rls["filtered_coefficients"], TOL_FIXED, "rls filtered coefficients");
    assert_eq!(fit.nobs_diffuse, rls["nobs_diffuse"].as_u64().unwrap() as usize);
...
fn tvp_mle_reaches_both_optimizers_and_flags_the_true_zero_variance() {
    ...
    assert!(fit.loglik >= ll_ref - TOL_MLE_LL, "loglik {} vs {ll_ref}", fit.loglik);
    let p_ref = as_f64_vec(&best["params"]);
    ...
    assert_eq!(fit.param_names, vec!["sigma2.irregular", "sigma2.const", "sigma2.x1", "sigma2.x2"]);
}
