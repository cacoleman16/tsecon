//! Shared helpers for the cointegration integration tests: fixture
//! loading, matrix conversion, and closeness assertions.
#![allow(dead_code)]
...
/// The `coint.json` `data` block is stored series-major (`k` rows of `T`
/// observations). Returns it transposed into the library's `T x k`
/// convention (oldest observation first, one column per series).
pub fn as_endog(v: &Value) -> Mat<f64> {
    let series = v.as_array().expect("expected JSON array of series");
    let k = series.len();
    let t = series[0].as_array().expect("expected JSON series").len();
    Mat::from_fn(t, k, |i, j| {
        series[j].as_array().expect("expected JSON series")[i]
            .as_f64()
            .expect("expected number")
    })
}
