//! Shared helpers for the integration tests: fixture loading, a faer matrix
//! builder, and a tiny seeded LCG (tests must not depend on tsecon-rng).
//!
...
/// Builds a faer matrix from column-major `Vec<Vec<f64>>` data.
pub fn mat_from_cols(cols: &[Vec<f64>]) -> Mat<f64> {
    let ncols = cols.len();
    let nrows = cols[0].len();
    Mat::from_fn(nrows, ncols, |i, j| cols[j][i])
}

/// Asserts `|a - b| <= tol` elementwise on slices; returns the max abs diff.
pub fn assert_slice_close(actual: &[f64], expected: &[f64], tol: f64, what: &str) -> f64 {
    ...
    for (i, (a, e)) in actual.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            d <= tol,
            "{what}[{i}]: {a} vs {e} (diff {d:e}, tol {tol:e})"
        );
    }
    max_diff
}
