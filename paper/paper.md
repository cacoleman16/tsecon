...
**121 functions** organized as a task-oriented facade over **41 Rust crates**,
...
The 121 functions span the applied workflow end to end:

- **Diagnostics and exploration**: `acf`, `pacf`, `ljung_box`, `jarque_bera`,
  `arch_lm`; the unit-root workflow `adf`, `kpss`, `check_stationarity`;
  spectral analysis (`periodogram`, `welch`, `coherence`); the one-call
  `check_series` battery that runs the families and returns
  evidence-backed model recommendations; and a specification-
...
suite (491 tests collected) whose fixtures are gated against `statsmodels`,
