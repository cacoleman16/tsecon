...
Outputs land in `out/`. The text summaries are committed; the run logs
(`*.log`, ignored repo-wide), the junit XML files and the 550 KB
`assertion_scan.json` are regenerated by the scripts above and are not.
